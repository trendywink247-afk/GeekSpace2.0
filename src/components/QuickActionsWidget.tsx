...
export function QuickActionsWidget({ collapsed = false, onToggleCollapse }: QuickActionsWidgetProps) {
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <Button
          ...
          className="w-14 h-14 rounded-full bg-[#A78BFA] hover:bg-[#00D4B0] shadow-lg shadow-[#A78BFA]/30"
        >
  ...
  return (
    <div className="glass-card-v2 border border-[#A78BFA]/20 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggleCollapse && (
            <button
              onClick={onToggleCollapse}
              className="text-[var(--ag-text-muted)] hover:text-white"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      <div className={`grid gap-2 transition-all duration-300 ${showAll ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {visibleActions.map((action) => (
          <button
            ...
            className="group flex flex-col items-center gap-2 p-3 rounded-xl bg-[var(--ag-bg-deep)] border border-[#A78BFA]/10 hover:border-[#A78BFA]/40 transition-all min-h-[80px]"
          >
            <div 
            ...
            <span className="text-xs text-[var(--ag-text-muted)] group-hover:text-[#E8E8F0] transition-colors">
              {action.label}
            </span>
            {action.shortcut && (
              <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] bg-[#0C0C18] rounded text-[var(--ag-text-muted)]">
                {action.shortcut}
              </kbd>
            )}
          </button>
      ...
      <div className="mt-3 pt-3 border-t border-[#A78BFA]/10">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-[#A78BFA]/10 hover:bg-[#A78BFA]/20 text-[var(--ag-cyan)] text-sm font-medium transition-colors"
        >
