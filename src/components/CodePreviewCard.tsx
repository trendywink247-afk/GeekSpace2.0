...
export function CodePreviewCard({ artifactId, title, html, css, js }: CodePreviewCardProps) {
  ...
  return (
    <div
      ...
      className="border border-[#7B61FF]/30 bg-[#05050A] rounded-xl overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#7B61FF]/20">
        <div className="flex items-center gap-2 min-w-0">
          <Code className="size-4 shrink-0 text-[#7B61FF]" />
          <span className="text-sm font-medium text-[#F4F6FF] truncate">
            {title}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            ...
            className="text-[#A7ACB8] hover:text-[#F4F6FF] hover:bg-[#7B61FF]/10"
          >
            {showPreview ? (
