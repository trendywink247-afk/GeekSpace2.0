  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl mx-4 bg-[#0A0A14] border border-[#8B5CF6]/20 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden relative"
      >
        {/* Top gradient border accent */}
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#8B5CF6]/60 to-transparent" />
        {/* Tab strip */}
        <div className="flex border-b border-[#8B5CF6]/10">
          <button
        ...
        {/* Header input */}
        <div className="flex items-center gap-3 px-4 py-4 border-b border-[#8B5CF6]/10">
          <Sparkles className="w-5 h-5 text-[#8B5CF6]/50 flex-shrink-0" />
          <input
            ref={inputRef}
            type="text"
            ...
            className="flex-1 bg-transparent text-[#E8E8F0] placeholder-[#4B5563] outline-none text-lg"
          />
          ...
          <div ref={listRef} className="max-h-[55vh] overflow-y-auto p-2">
            {flatCommands.length === 0 ? (
              ...
              Object.entries(groupedCommands).map(([category, items], catIdx) => (
                ...
                >
                  <div className="px-3 py-2 text-[10px] font-semibold text-[#B8C4D4]/70 uppercase tracking-[0.12em]">
                    {category}
                  </div>
                      ...
                      <motion.button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 ${
                          isSelected
                            ...
                            : 'hover:bg-[#8B5CF6]/5 border-l-2 border-transparent'
                        }`}
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors duration-150 ${
                          isSelected ? 'bg-[#8B5CF6]/20 text-[#8B5CF6]' : 'bg-[#06060B] text-[#6B7280]'
                        }`}>
                          {cmd.icon}
                        </div>
                        <div className="flex-1 text-left min-w-0">
                          <div className={`font-medium text-sm ${isSelected ? 'text-[#E8E8F0]' : 'text-[#B8C4D4]'}`}>
                            {cmd.title}
                          </div>
                          {cmd.subtitle && (
              ...
              <div className="p-2">
                {dataTypeOrder
                  ...
                  .map((type, groupIdx) => (
                    ...
                    >
                      <div className="px-3 py-2 text-[10px] font-semibold text-[#B8C4D4]/70 uppercase tracking-[0.12em] flex items-center gap-2">
                        ...
                        return (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors duration-150 ${
                              isSelected ? 'bg-[#8B5CF6]/20' : 'bg-[#06060B]'
                            }`}>
                              {DATA_TYPE_ICONS[r.type] ?? <Search className="w-4 h-4 text-[#6B7280]" />}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className={`text-sm truncate font-medium ${isSelected ? 'text-[#E8E8F0]' : 'text-[#E8E8F0]'}`}>
                                {r.title}
                              </p>
              ...
              <div className="p-6">
                ...
                {recentSearches.length > 0 && (
                  <div className="border-t border-[#1A1A2E] pt-4">
                    <p className="text-xs text-[#B8C4D4] uppercase tracking-wider mb-2 px-2">Recent searches</p>
                    <div className="flex flex-wrap gap-2 px-2">
                      {recentSearches.slice(0, 6).map((s, i) => (
                        <button
                          key={i}
                          onClick={() => setSearch(s)}
                          className="px-3 py-1.5 text-xs text-[#B8C4D4] bg-[#0C0C18] border border-[#1A1A2E] rounded-lg hover:border-[#8B5CF6]/30 hover:text-[#E8E8F0] transition-colors"
                        >
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-5 px-4 py-2.5 border-t border-[#8B5CF6]/10 text-[11px] text-[#4B5563]">
