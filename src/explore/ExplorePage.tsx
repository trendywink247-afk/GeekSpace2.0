...
export function ExplorePage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#06060B]/80 backdrop-blur-xl border-b border-[#8B5CF6]/20" style={{ backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)' }}>
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(isAuthenticated ? '/dashboard' : '/')} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-[#8B5CF6]/10 transition-colors focus-visible:ring-2 focus-visible:ring-[#8B5CF6]/50" aria-label="Go back">
          ...
          {!isAuthenticated && (
            <Button onClick={() => navigate('/login')} className="bg-[#8B5CF6] hover:bg-[#7C3AED]">
              Get Your Space
            </Button>
          )}
        </div>
      ...
      <main className="pt-24 pb-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="mb-8 space-y-4">
          <div className="relative max-w-xl mx-auto">
            ...
            <Input
              ...
              className="pl-12 h-14 md:h-12 bg-[#0C0C18] border-[#8B5CF6]/30 text-[#E8E8F0] rounded-xl text-base"
            />
          </div>
            ...
            <div className="flex flex-nowrap md:flex-wrap md:justify-center gap-2 pb-2 md:pb-0">
              {allTags.map((tag) => (
                <button
                  ...
                  className={`px-4 py-1.5 min-h-[44px] rounded-full text-sm transition-all whitespace-nowrap shrink-0 press-scale focus-visible:ring-2 focus-visible:ring-[#8B5CF6]/50 ${
                    activeTag === tag
                      ? 'bg-[#8B5CF6] text-white'
                      : 'bg-[#0C0C18] border border-[#8B5CF6]/20 text-[#9CA3AF] hover:border-[#8B5CF6]/50'
                  }`}
                >
                  {tag}
                </button>
        ...
        {isLoading && profiles.length === 0 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {profiles.map((profile, i) => (
              <div
                ...
                className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:border-[#8B5CF6]/50 transition-all group relative overflow-hidden press-scale"
              >
                ...
                <div className="relative mb-4">
                  ...
                  {profile.agentEnabled && (
                    <div className="absolute bottom-0 right-0 w-5 h-5 rounded-full bg-[#10B981] border-2 border-[#0C0C18] flex items-center justify-center">
                ...
                {/* Info */}
                <h3 className="font-semibold text-[#E8E8F0] text-lg group-hover:text-[#8B5CF6] transition-colors">
                  {profile.name}
                </h3>
                ...
                <div className="flex flex-wrap gap-1 mt-3">
                  {profile.tags.slice(0, 3).map((tag) => (
                    <Badge key={tag} variant="outline" className="border-[#8B5CF6]/20 text-[#9CA3AF] text-xs">
                      {tag}
                    </Badge>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {profile.skills.slice(0, 4).map((skill) => (
                    <span key={skill} className="text-xs text-[#8B5CF6]/60">{skill}</span>
                  ))}
                </div>

                {/* Action buttons */}
                <div className="mt-4 pt-3 border-t border-[#8B5CF6]/10 flex gap-2">
                  {isAuthenticated && profile.agentEnabled && (
                    <button
                      onClick={(e) => handleAgentChat(e, profile)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 min-h-[44px] rounded-lg bg-[#10B981]/10 border border-[#10B981]/20 text-xs text-[#10B981] hover:bg-[#10B981]/20 transition-colors press-scale focus-visible:ring-2 focus-visible:ring-[#8B5CF6]/50"
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 min-h-[44px] rounded-lg bg-[#06060B] border border-[#8B5CF6]/20 text-xs text-[#9CA3AF] hover:text-[#E8E8F0] hover:border-[#8B5CF6]/40 transition-colors press-scale focus-visible:ring-2 focus-visible:ring-[#8B5CF6]/50"
                  >
        ...
        {!isLoading && profiles.length === 0 && (
          <div className="text-center py-20">
            <Search className="w-12 h-12 text-[#8B5CF6]/30 mx-auto mb-4" />
            <p className="text-[#9CA3AF]">{loadError || 'No people found matching your search'}</p>
            {loadError && (
              <button
                onClick={fetchProfiles}
                className="mt-4 px-4 py-2 rounded-xl bg-[#8B5CF6]/20 hover:bg-[#8B5CF6]/30 text-[#8B5CF6] text-sm font-medium transition-colors"
              >
                Retry
              </button>
            )}
