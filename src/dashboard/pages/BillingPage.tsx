      ...
      >
        ...
        <div className="px-5 pb-5 space-y-4">
          ...
          <div className="flex items-baseline flex-wrap gap-2">
            ...
            {plan.priceUsd > 0 && (
              <span className="text-sm text-[var(--ag-text-secondary)]">/ {plan.intervalLabel}</span>
            )}
          </div>
          <div className="text-sm text-[var(--ag-text-secondary)]">{plan.description}</div>

          {/* Features */}
          <div className="space-y-2 py-2 border-t border-[rgba(139,92,246,0.08)]">
            <div className="flex items-center justify-between text-sm">
              <span className="text-[var(--ag-text-secondary)]">Agent Slots</span>
              <span className="text-[var(--ag-text-primary)] font-medium">{display.agentSlots}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-[var(--ag-text-secondary)]">Token Budget</span>
          ...
          {isCurrent ? (
            ...
            <div>
              ...
              <p className="text-xs text-[var(--ag-text-secondary)] text-center mt-1.5">UPI, Cards, Net Banking accepted</p>
            </div>
          ) : (
            <Button
      ...
      <BlurFade delay={0.2}>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            <h2 className="text-base font-semibold text-[var(--ag-text-primary)]">Premium Subscription</h2>
        ...
        <div className="overflow-x-auto -mx-4 px-4 md:-mx-5 md:px-5">
          <table className="w-full min-w-[500px]">
            ...
            <tbody>
              ...
              <tr>
                <td className="py-3 px-2 text-sm text-[var(--ag-text-secondary)]">Price</td>
                {plans.map((plan) => {
                  const display = PLAN_DISPLAY[plan.id];
                  return (
                    <td key={plan.id} className="text-center py-3 px-2">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        {display?.oldPrice > 0 && (
                          <span className="text-xs text-[var(--ag-text-secondary)] line-through">{oldPrice(plan)}</span>
                        )}
                      </div>
                    </td>
                  );
      ...
      <BlurFade delay={0.6}>
        ...
        {usage.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-[var(--ag-text-secondary)]">No usage data yet</p>
            <p className="text-sm text-[var(--ag-text-secondary)]">Start chatting and usage will appear here</p>
          </div>
        ) : (
          <MobileTable<DailyUsage>
            columns={[
              ...
              { key: 'calls', label: 'Calls', render: (row) => <span className="text-[var(--ag-text-secondary)] font-mono">{row.calls}</span> },
              { key: 'tokens', label: 'Tokens', render: (row) => <span className="text-[var(--ag-text-secondary)] font-mono">{(row.total_tokens ?? 0).toLocaleString()}</span> },
      ...
      <BlurFade delay={0.7}>
        ...
        {events.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-[var(--ag-text-secondary)]">No credit events yet</p>
            <p className="text-sm text-[var(--ag-text-secondary)]">Credit usage will appear here after your first AI request</p>
          </div>
        ) : (
          <MobileTable<UsageEvent>
            columns={[
              ...
              },
              {
                key: 'channel',
                label: 'Channel',
                render: (row) => <span className="text-[var(--ag-text-secondary)] capitalize">{row.channel || '\u2014'}</span>,
              },
              {
                key: 'model',
                label: 'Model',
                render: (row) => (
                  <span className="text-[var(--ag-text-secondary)] font-mono text-xs truncate max-w-[100px] block">
              ...
              },
              {
                ...
                render: (row) => {
                  ...
                  return <span className="text-[var(--ag-text-secondary)] font-mono">{(tokensIn + tokensOut).toLocaleString()}</span>;
                },
              },
              {
                key: 'cost_usd',
