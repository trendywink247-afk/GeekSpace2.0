...
export function useFocusSession(): UseFocusSessionReturn {
	...
	const { elapsed, remaining, progress } = useTimer(
		...
		activeSession?.duration_min ?? null,
	);
	...
	const loadSession = useCallback(async () => {
		try {
			const [sRes, nsRes, dRes] = await Promise.all([
	...
	const start = useCallback(
		async (goal: string | null, durationMin: number) => {
			setLoading(true);
			try {
	...
	const end = useCallback(
		async (completed = true) => {
			...
			try {
				...
				toast.success(
					completed ? "Focus session completed" : "Focus session ended",
				);
				void notifyDone(
					completed ? "Focus session completed" : "Focus session ended",
				);
	...
	const toggleFocusMode = useCallback(async () => {
		...
		try {
			const res = await api.patch("/focus/settings", {
				focus_mode_active: newVal,
			});
