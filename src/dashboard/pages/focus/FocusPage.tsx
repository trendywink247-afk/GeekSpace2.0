...
// Timer upper half, controls/tabs below
...
export function FocusPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [hRes] = await Promise.all([api.get('/habits'), loadSession(), loadHistory()]);
  ...
  useEffect(() => {
    if (prevRemaining.current !== null && prevRemaining.current > 0 && remaining === 0 && session) {
      if ('Notification' in window && Notification.permission === 'granted')
        new Notification('Focus Session Complete!', {body:`Great work! You focused for ${session.duration_min??0} minutes.`,icon:'/favicon.ico'});
      void end(true);
  ...
  async function handleStartFocus() {
    await start(goalInput||null, durInput);
    setShowStartModal(false); setGoalInput(''); setShowBreakSuggestion(false);
  }
  ...
  async function handleLogHabit(id: number) {
    try {
      ...
      setHabits((prev) => {
        const updated = prev.map((h) => h.id===id ? {...h,logged_today:true,current_streak:h.current_streak+1} : h);
        if (updated.length>0 && updated.every((h)=>h.logged_today)) { setShowCelebration(true); setTimeout(()=>setShowCelebration(false),2000); }
        return updated;
      });
  ...
  async function handleAddHabit() {
    ...
    try {
      await api.post('/habits', {name:newHabitName.trim(),icon:newHabitIcon,frequency:newHabitFreq});
      setNewHabitName(''); setNewHabitIcon('⭐'); setNewHabitFreq('daily'); setShowAddHabit(false);
      void load(); toast.success('Habit added');
  ...
  async function handleDeleteHabit(id: number) {
    ...
    try { await api.delete('/habits/'+id); void load(); toast.success('Habit deleted'); }
  ...
  if (initialLoading) return (
    <div className="max-w-2xl mx-auto p-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[1,2,3].map((i)=><div key={i} className="h-20 rounded-2xl animate-pulse" style={{background:'var(--ag-bg-surface)'}}/>)}
      </div>
    ...
    <DashboardPageWrapper>
      <PageShell>
        <div className="max-w-2xl mx-auto space-y-6 pb-24 md:pb-6">
            ...
            <PageHeader icon={Target} title="Focus & Habits" subtitle="Coached by Pulse"
              ...
              actions={
                <motion.div whileTap={{scale:0.96}}>
                  <Button variant="ghost" size="sm" onClick={()=>void toggleFocusMode()}
                    className="gap-1.5 text-xs min-h-[44px] px-3 rounded-xl transition-all focus-visible:outline-none"
                    style={{background:settings?.focus_mode_active?'rgba(99,102,241,0.12)':'transparent',color:settings?.focus_mode_active?'var(--ag-indigo)':'var(--ag-text-secondary)',boxShadow:settings?.focus_mode_active?'0 0 0 1px rgba(99,102,241,0.22)':'none'}}
          ...
          <AnimatePresence>
            {deferredCount > 0 && (
              <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{type:'spring',duration:0.35,bounce:0}}>
                <GlassCard accent="indigo" className="px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-sm" style={{color:'var(--ag-text-primary)'}}>
                      <Bell size={14} className="inline mr-1.5" style={{color:'var(--ag-indigo)'}}/>
                      {deferredCount} message{deferredCount>1?'s':''} held during focus
                    </span>
                    <Button size="sm" variant="outline" className="text-xs min-h-[44px] rounded-lg flex-shrink-0"
                      style={{borderColor:'var(--ag-border-default)',color:'var(--ag-indigo)'}} onClick={()=>void load()}>
                      View now
                    </Button>
                  </div>
                </GlassCard>
          ...
          <BlurFade delay={0.22}>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full p-1 h-12 rounded-xl" style={{background:'var(--ag-bg-surface)',boxShadow:'0 0 0 1px rgba(139,92,246,0.08)'}}>
                <TabsTrigger value="focus" className="flex-1 rounded-lg h-10 text-sm font-medium transition-colors min-h-[44px]">
                  <Timer size={15} className="mr-1.5"/>Focus Sessions
                </TabsTrigger>
                <TabsTrigger value="habits" className="flex-1 rounded-lg h-10 text-sm font-medium transition-colors min-h-[44px]">
                  <Flame size={15} className="mr-1.5"/>Daily Habits
                </TabsTrigger>
              </TabsList>

              <TabsContent value="focus" className="mt-4 space-y-4">
                {session ? (
                  <FocusTimer session={session} progress={progress} remaining={remaining} remainStr={remainStr} elapsedStr={elapsedStr} isLoading={isLoading} onEnd={()=>end(true)}/>
                ) : showBreakSuggestion ? (
                  <BreakReminder onDismiss={()=>setShowBreakSuggestion(false)}/>
                ) : (
                  <SessionStarter durInput={durInput} setDurInput={setDurInput} onOpenModal={()=>setShowStartModal(true)}/>
                )}
