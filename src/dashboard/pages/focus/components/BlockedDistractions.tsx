// BlockedDistractions — blocked sites/apps list
import { GlassCard, SectionHeader, Chip } from '@/components/ui/agentin';
import { ShieldOff } from 'lucide-react';

const DEFAULT_BLOCKS = ['Social feeds', 'News sites', 'Video streaming'];

export function BlockedDistractions({ blockedSites }: { blockedSites?: string }) {
  const sites = blockedSites ? blockedSites.split(',').map((s)=>s.trim()).filter(Boolean) : DEFAULT_BLOCKS;
  return (
