...
export function useOfficeData(): UseOfficeDataReturn {
		...
		try {
			...
			if (res.status === 429) {
				...
				const expBackoffMs = Math.min(
					60_000,
					3_000 * 2 ** consecutiveErrorsRef.current,
				);
			...
			const metrics: OfficeMetrics = {
				...
				providerBreakdown:
					(metricsRaw.providerBreakdown as Record<string, number>) ?? {},
			};
			...
			setOfficeData({
				...
				metrics,
				delegationStatus:
					(data.delegationStatus as DelegationStatus | undefined) ?? null,
			});
			...
			if (recentEvents && recentEvents.length > 0) {
				...
				if (newEvents.length > 0) {
					setSseEvents((prev) => {
						...
						return next.length > TIMELINE_MAX_ITEMS
							? next.slice(-TIMELINE_MAX_ITEMS)
							: next;
					});
				}
	...
	useEffect(() => {
		...
		// GS-011: SSE already connected above — first poll immediately
		schedule(0);
