...
import type { AgentPerception } from '../../world/perception';
...
import {
  GROUP_MEETING_BEHAVIORS,
  getActiveGroupMeeting,
  setActiveGroupMeeting,
  getActiveGroupId,
  setActiveGroupId,
} from './groupMeeting';
...
// activeGroupId state moved to systems/behavior/groupMeeting.ts (use getActiveGroupId/setActiveGroupId)
...
// ── Group meeting logic ─────────────────────────────────────────────────────
// GroupMeeting interface, activeGroupMeeting state, and GROUP_MEETING_BEHAVIORS
// are defined in systems/behavior/groupMeeting.ts and imported above.
// tryStartGroupMeeting() and tickGroupMeeting() remain here due to tight coupling
// with internal helpers (behaviorStates, computeFacing, chooseDestination, etc.).
...
function tryStartGroupMeeting(idleAgents: CanvasAgent[]): void {
  if (getActiveGroupMeeting()) return;
...
function tickGroupMeeting(
  ...
  if (!getActiveGroupMeeting()) return { targets };

  const gm = getActiveGroupMeeting()!;
  ...
  const updatedAgents = agents.map((agent) => {
    ...
    switch (bState.mode) {
      ...
      case 'group-meeting': {
        ...
        const _agm = getActiveGroupMeeting();
        if (_agm) {
          const assigned = _agm.assignedPoints.get(agent.id);
          bState.facing = assigned?.facing ?? computeFacing(
            agent.x, agent.y,
            _agm.centerPoint.x, _agm.centerPoint.y,
          );
        }
        // If group was dispersed but we're still in this mode, roam freely
        if (!getActiveGroupId() || bState.groupId !== getActiveGroupId()) {
...
// AgentDelivery interface and pendingDeliveries array moved to
// systems/behavior/deliveries.ts and imported above.
