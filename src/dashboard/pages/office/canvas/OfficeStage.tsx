...
import {
	BUBBLE_PRIORITY,
	SpeechBubbleLayer,
} from "../overlays/SpeechBubbleLayer";
import {
	ATTENTION_PHRASES,
	COLLAB_RECV_PHRASES,
	COLLAB_SEND_PHRASES,
...
const USER_ANCHOR = { x: 13, y: 21 }; // walkable tile below meeting room; (13,22) was blocked
...
interface LaunchHuddle {
	...
	seatAssignments: Map<
		AgentId,
		{ x: number; y: number; facing: "down" | "up" | "left" | "right" }
	>;
	leadAgent: AgentId;
	startedAt: number;
}
...
function getAgentDesk(id: AgentId): { x: number; y: number } {
	if (id in CORE_DESK_POSITIONS)
		return CORE_DESK_POSITIONS[id as keyof typeof CORE_DESK_POSITIONS];
	if (id in SPECIALIST_POSITIONS)
		return SPECIALIST_POSITIONS[id as keyof typeof SPECIALIST_POSITIONS];
	return { x: 7, y: 14 };
}

function getSeatPosition(pos: { x: number; y: number }): {
	x: number;
	y: number;
} {
	if (isBlocked(pos.x, pos.y)) {
		const v = nearestWalkable(pos.x, pos.y);
		if (v) return v;
	}
	return pos;
}
...
export default function OfficeStage({
	...
	useEffect(() => {
		...
		idleChatterRef.current.nextChatterDelay =
			IDLE_CHATTER_MIN_MS +
			Math.random() * (IDLE_CHATTER_MAX_MS - IDLE_CHATTER_MIN_MS);
	}, []);
	...
	useEffect(() => {
		...
		if (current > target) {
			effectStateRef.current.particles = effectStateRef.current.particles.slice(
				0,
				target,
			);
			...
			effectStateRef.current.particles = [
				...effectStateRef.current.particles,
				...extra,
			];
		}
	...
	useEffect(() => {
		Promise.all([loadOfficeAssets(), loadSpriteSheets()])
			.then(() => {
				initAmbientParticles(window.innerWidth < 768 ? 5 : 15);
				setAssetsReady(true);
			})
	...
	const addBubble = useCallback(
		(
			agentId: AgentId,
			text: string,
			opts?: { interactive?: boolean; priority?: number; delay?: number },
		) => {
			...
			const doAdd = () => {
				const existing = bubblesRef.current.find(
					(b) =>
						b.agentId === agentId &&
						(b.priority ?? BUBBLE_PRIORITY.SOCIAL) > priority &&
						Date.now() < b.expiresAt,
				);
				if (existing) {
					setTimeout(
						doAdd,
						Math.min(existing.expiresAt - Date.now() + 100, 3000),
					);
					return;
				}
				const now = Date.now();
				...
				() => {
					setBubbles((prev) => [
						...prev.slice(-(MAX_SPEECH_BUBBLES - 1)),
						{
							...
							agentId: id,
							text: GREETING_PHRASES[id] || "Hello!",
							color: AGENT_COLORS[id] || "#A78BFA",
							createdAt: Date.now(),
							expiresAt: Date.now() + SPEECH_BUBBLE_TTL + 1000,
							priority: BUBBLE_PRIORITY.SOCIAL,
						},
					]);
				},
				1500 + i * 600,
				...
				switch (evt.state as AgentStateType) {
					...
					case "task_failed": {
						...
						if (evt.state === "thinking") {
							...
							addBubble(agentId, randomPhrase(THINKING_PHRASES, agentId), {
							...
							if (evt.isMultiAgent) {
								if (!activeLaunchHuddle) {
									...
									activeLaunchHuddle.seatAssignments.set(
										agentId,
										MEETING_TABLE_SEATS[4],
									);
									const s = MEETING_TABLE_SEATS[4];
									agent.targetX = s.x;
									agent.targetY = s.y;
									agent.path = findFullPath(agent.x, agent.y, s.x, s.y);
									agent.pathIndex = 0;
									cancelIdleBehavior(agentId);
								} else if (!activeLaunchHuddle.agents.has(agentId)) {
									activeLaunchHuddle.agents.add(agentId);
									const used = new Set(
										[...activeLaunchHuddle.seatAssignments.values()].map(
											(s) => `${s.x},${s.y}`,
										),
									);
									const free = MEETING_TABLE_SEATS.find(
										(s) => !used.has(`${s.x},${s.y}`),
									);
									if (free) {
										activeLaunchHuddle.seatAssignments.set(agentId, free);
										agent.targetX = free.x;
										agent.targetY = free.y;
										agent.path = findFullPath(agent.x, agent.y, free.x, free.y);
										agent.pathIndex = 0;
										cancelIdleBehavior(agentId);
									}
								}
						...
						if (evt.state === "task_completed") {
							addBubble(agentId, randomPhrase(COMPLETION_PHRASES, agentId), {
								priority: BUBBLE_PRIORITY.TASK_COMPLETE,
							});
							...
							if (del) {
								...
								if (dIdx !== -1) {
									...
									const phrases = DELEGATION_REACTION_PHRASES[
										del.delegatorId
									] || ["{name} finished!"];
									...
									addBubble(del.delegatorId, phrase, {
										priority: BUBBLE_PRIORITY.TASK_COMPLETE,
									});
						...
						if (evt.state === "task_failed") {
							addBubble(agentId, randomPhrase(FAILURE_PHRASES, agentId), {
								priority: BUBBLE_PRIORITY.TASK_FAILURE,
							});
						}
					...
					case "error": {
						...
						addBubble(agentId, randomPhrase(ERROR_PHRASES, agentId), {
							priority: BUBBLE_PRIORITY.TASK_FAILURE,
						});
						if (!isMobileRef.current)
							triggerConfetti(confettiRef.current, CANVAS_W, "broken");
						break;
					}
					...
					case "message_in": {
						...
						if (Math.abs(dxM) > Math.abs(dyM))
							agent.facing = dxM > 0 ? "right" : "left";
						else agent.facing = dyM > 0 ? "down" : "up";
						agent.fx = { ...agent.fx, glowStart: Date.now() };
						const rank = [...CORE_AGENTS, ...SPECIALIST_AGENTS].indexOf(
							agentId,
						);
						if (rank < 3)
							addBubble(agentId, randomPhrase(ATTENTION_PHRASES, agentId), {
								priority: BUBBLE_PRIORITY.SOCIAL,
							});
						break;
					}
					...
					case "message_out": {
						...
						if (evt.content) {
							addBubble(agentId, evt.content.slice(0, 60), {
								interactive: evt.content.length > 60,
								priority: BUBBLE_PRIORITY.TOOL_CALL,
							});
						}
						break;
					...
					case "goal_started": {
						...
						addBubble(agentId, randomPhrase(GOAL_START_PHRASES, agentId), {
							priority: BUBBLE_PRIORITY.TASK_COMPLETE,
						});
						if (!activeLaunchHuddle) {
							activeLaunchHuddle = {
							...
							agent.targetX = gs.x;
							agent.targetY = gs.y;
							agent.path = findFullPath(agent.x, agent.y, gs.x, gs.y);
							agent.pathIndex = 0;
							cancelIdleBehavior(agentId);
						}
						break;
					...
					case "goal_completed": {
						...
						addBubble(agentId, randomPhrase(GOAL_COMPLETION_PHRASES, agentId), {
							priority: BUBBLE_PRIORITY.TASK_COMPLETE,
						});
						if (!isMobileRef.current)
							triggerConfetti(confettiRef.current, CANVAS_W, "celebration");
						...
						for (let ni = 0; ni < next.length; ni++)
							next[ni] = {
								...next[ni],
								fx: { ...next[ni].fx, bounceStart: Date.now() },
							};
						setTimeout(() => clearEffects(effectStateRef.current), 4000);
						if (activeLaunchHuddle) {
							activeLaunchHuddle.agents.clear();
							activeLaunchHuddle.seatAssignments.clear();
					...
					case "streak_milestone": {
						agent.state = "idle" as AgentStateType;
						agent.fx = { ...agent.fx, bounceStart: Date.now() };
						addBubble(agentId, "🔥 Streak milestone!", {
							priority: BUBBLE_PRIORITY.TASK_COMPLETE,
						});
						if (!isMobileRef.current)
							triggerFireworks(fireworksRef.current, CANVAS_W, CANVAS_H);
						break;
					}
					...
					case "delegating": {
						...
						addBubble(
							agentId,
							`${targetId ? targetId.charAt(0).toUpperCase() + targetId.slice(1) : "team"}: ${evt.content ? evt.content.slice(0, 30) : "task"}`,
							{ priority: BUBBLE_PRIORITY.SOCIAL },
						);

						if (
							targetId &&
							SPECIALIST_AGENTS.includes(targetId as SpecialistId)
						) {
							const sIdx = next.findIndex((a) => a.id === targetId);
							if (sIdx !== -1) {
								const spec = { ...next[sIdx] };
								spec.state = "task_started";
								// NO TELEPORT — specialist walks from wherever they are.
								// Try walk-to-user first; if unreachable (shouldn't happen now), fallback to desk.
								const dk2 = getAgentDesk(targetId as AgentId);
								const dvs = validateTarget(dk2.x, dk2.y, spec.x, spec.y);
								const userPath = findFullPath(
									spec.x,
									spec.y,
									USER_ANCHOR.x,
									USER_ANCHOR.y,
								...
								if (userPath.length > 0) {
									spec.path = userPath;
									spec.pathIndex = 0;
									spec.targetX = USER_ANCHOR.x;
									spec.targetY = USER_ANCHOR.y;
									spec.fx = { ...spec.fx, pendingDeskTarget: dvs };
									addBubble(
										targetId as AgentId,
										randomPhrase(WALK_TO_USER_PHRASES, targetId),
										{
											priority: BUBBLE_PRIORITY.SOCIAL,
										},
									);
								} else {
									// Fallback — walk straight to own desk (shouldn't happen with walkable USER_ANCHOR)
									spec.targetX = dvs.x;
									spec.targetY = dvs.y;
									spec.path = findFullPath(spec.x, spec.y, dvs.x, dvs.y);
									spec.pathIndex = 0;
									const rp =
										COLLAB_RECV_PHRASES[targetId] ??
										COLLAB_RECV_PHRASES["weebo"];
									const ctx2 = evt.content ? evt.content.slice(0, 25) : null;
									addBubble(
										targetId as AgentId,
										ctx2
											? `On it: ${ctx2}...`
											: rp[Math.floor(Math.random() * rp.length)],
										{ priority: BUBBLE_PRIORITY.SOCIAL },
									);
								}
								next[sIdx] = spec;
					...
					case "comm_sent": {
						...
						addBubble(agentId, randomPhrase(COLLAB_SEND_PHRASES, agentId), {
							priority: BUBBLE_PRIORITY.SOCIAL,
						});
						break;
					}
					...
					case "comm_received": {
						...
						addBubble(agentId, randomPhrase(COLLAB_RECV_PHRASES, agentId), {
							priority: BUBBLE_PRIORITY.SOCIAL,
						});
						break;
					}
					...
					case "done": {
						agent.state = "done";
						{
							...
							setTimeout(
								() => clearEffects(effectStateRef.current),
								clearDelay,
							);
						}
						if (activeLaunchHuddle?.agents.has(agentId)) {
							...
							agent.targetX = dt3.x;
							agent.targetY = dt3.y;
							agent.path = findFullPath(agent.x, agent.y, dt3.x, dt3.y);
							agent.pathIndex = 0;
		...
		const frame = (time: number) => {
			...
			if (behaviorAccum >= BEHAVIOR_INTERVAL) {
				...
				if (tickRef.current % 300 === 0) {
					const now = Date.now();
					for (const [k, v] of delegationTracker)
						if (now - v.timestamp > 5 * 60 * 1000) delegationTracker.delete(k);
				}
				...
				// Idle chatter (30s+ quiet, 2+ agents nearby)
				{
					const now = Date.now();
					if (
						now - idleChatterRef.current.lastEventTime >
							IDLE_CHATTER_QUIET_MS &&
						now - idleChatterRef.current.lastChatterTime >
							idleChatterRef.current.nextChatterDelay
					) {
						...
						for (let i = 0; i < all.length; i++)
							for (let j = i + 1; j < all.length; j++) {
								if (
									Math.abs(all[i].x - all[j].x) +
										Math.abs(all[i].y - all[j].y) <=
									4
								)
						...
						if (pairs.length > 0) {
							...
							addBubble(
								ini.id,
								randomPhrase(IDLE_CHATTER_PHRASES.initiator, ini.id),
								{
									priority: BUBBLE_PRIORITY.SOCIAL,
								},
							);
							addBubble(
								res.id,
								randomPhrase(IDLE_CHATTER_PHRASES.responder, res.id),
								{
									priority: BUBBLE_PRIORITY.SOCIAL,
									delay: 2000,
								},
				...
				setAgents(() => {
					...
					const next = base.map((agent) => {
						...
						if (
							agent.fx?.pendingDeskTarget &&
							agent.path.length === 0 &&
							agent.x === USER_ANCHOR.x &&
							agent.y === USER_ANCHOR.y
						) {
							const dest = agent.fx.pendingDeskTarget;
							const { pendingDeskTarget: _d, ...restFx } = agent.fx;
							void _d;
							const rp2 =
								COLLAB_RECV_PHRASES[agent.id as AgentId] ??
								COLLAB_RECV_PHRASES["weebo"];
							addBubble(agent.id, rp2[Math.floor(Math.random() * rp2.length)], {
								priority: BUBBLE_PRIORITY.SOCIAL,
							});
							changed = true;
							return {
								...
								targetX: dest.x,
								targetY: dest.y,
								path: findFullPath(agent.x, agent.y, dest.x, dest.y),
								pathIndex: 0,
							};
						}
						...
						if (
							...
							if (fp.length > 0) {
								changed = true;
								return { ...agent, path: fp, pathIndex: 0 };
							}
						}
						...
						if (agent.path.length > 0 && agent.pathIndex < agent.path.length) {
							...
							const d = Math.sqrt(
								(step.x * CELL + CELL / 2 - agent.renderX) ** 2 +
									(step.y * CELL + CELL / 2 - agent.renderY) ** 2,
							);
							if (d < 2) {
								changed = true;
								const u = {
								...
								if (u.pathIndex >= u.path.length) {
									u.path = [];
									u.pathIndex = 0;
									u.renderX = u.x * CELL + CELL / 2;
									u.renderY = u.y * CELL + CELL / 2;
								} else {
									u.targetX = u.path[u.pathIndex].x;
									u.targetY = u.path[u.pathIndex].y;
								}
								return u;
							}
						}
					...
					if (newBubbles.length > 0) {
						setBubbles((b) => [
							...
							...newBubbles.map((nb) => ({
								...nb,
								priority: nb.priority ?? BUBBLE_PRIORITY.SOCIAL,
							})),
						]);
					}
			...
			if (expireAccum >= BEHAVIOR_INTERVAL) {
				...
				const now = Date.now();
				setBeams((p) => {
					const f = p.filter((b) => now - b.createdAt < b.duration);
					return f.length === p.length ? p : f;
				});
				setBubbles((p) => {
					const f = p.filter((b) => now < b.expiresAt);
					return f.length === p.length ? p : f;
				});
			}
			...
			for (let i = 0; i < ref.length; i++) {
				const a = ref[i];
				const tpx =
					a.path.length > 0 && a.pathIndex < a.path.length
						? a.path[a.pathIndex].x * CELL + CELL / 2
						: a.x * CELL + CELL / 2;
				const tpy =
					a.path.length > 0 && a.pathIndex < a.path.length
						? a.path[a.pathIndex].y * CELL + CELL / 2
						: a.y * CELL + CELL / 2;
				const dx = tpx - a.renderX,
					dy = tpy - a.renderY;
	...
	const hitTestAgent = useCallback((ox: number, oy: number): AgentId | null => {
		...
		for (const a of agentsRef.current)
			if (Math.abs(a.x - cx) <= 2 && Math.abs(a.y - cy) <= 2) return a.id;
		return null;
	}, []);
	...
	const hitTestObject = useCallback(
		(
			...
			for (const o of SMART_OBJECTS) {
				if (
					o.footprint.some((f) => f.x === cx && f.y === cy) ||
					o.interactionPoints.some((ip) => ip.x === cx && ip.y === cy)
				)
		...
		(e: React.MouseEvent<HTMLCanvasElement>) => {
			...
			clickTimerRef.current = setTimeout(() => {
				...
				if (hit) {
					...
					const obj = hitTestObject(ox, oy);
					if (obj && onObjectClick) onObjectClick(obj.id, obj.type, obj.label);
	...
	const handleMouseMove = useCallback(
		(e: React.MouseEvent<HTMLCanvasElement>) => {
			...
			hoverRafRef.current = requestAnimationFrame(() => {
				...
				const ox = (cx - rect.left) * (CANVAS_W / rect.width),
					oy = (cy - rect.top) * (CANVAS_H / rect.height);
				canvas.style.cursor =
					hitTestAgent(ox, oy) || hitTestObject(ox, oy) ? "pointer" : "default";
			});
		},
