...
interface Props {
  ...
  /** Shift HUD up on mobile to clear bottom sheet */
...
export function SpotlightHUD({ agent, taskCount, onChat, onAssignTask, onDismiss, officeData, mobileBottomOffset }: Props) {
  ...
  return (
    <motion.div
      className={`absolute left-1/2 -translate-x-1/2 z-20 w-[calc(100%-2rem)] max-w-xl ${mobileBottomOffset ? 'bottom-16' : 'bottom-4'}`}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ type: 'spring', stiffness: 300, damping: 25 }}
    >
      <div
        className="flex flex-col sm:flex-row sm:flex-nowrap items-stretch sm:items-center gap-2 sm:gap-3 rounded-xl px-3 sm:px-4 py-2.5"
        style={{
      ...
      >
        ...
        {/* Separator — hidden on mobile (stacked layout) */}
        <div className="hidden sm:block w-px h-6" style={{ background: `${color}20` }} />

        {/* Actions */}
        <div className="flex items-center gap-1.5 w-full sm:w-auto">
          {showChatInput ? (
            <form onSubmit={handleChatSubmit} className="flex gap-1 flex-1">
              <input
                autoFocus
                value={chatText}
                onChange={(e) => setChatText(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Escape') { setShowChatInput(false); setChatText(''); } }}
                placeholder={`Message ${agent.name.split(' ').pop()}...`}
                className="px-2.5 py-1.5 text-xs rounded-lg flex-1 min-w-0 outline-none placeholder:text-[#4B5563]"
                style={{
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="px-3 py-1.5 rounded-lg text-[11px] font-medium transition-colors min-h-[36px] sm:min-h-[44px] flex-1 sm:flex-initial"
              style={{
                background: `${color}10`,
                border: `1px solid ${color}30`,
                color,
          ...
          {showTaskInput ? (
            <form onSubmit={handleTaskSubmit} className="flex gap-1 flex-1">
              <input
                autoFocus
                value={taskText}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="px-1.5 py-1 rounded-lg text-[11px] text-[#4B5563] hover:text-[#9CA3AF] transition-colors min-h-[36px] sm:min-h-[44px] min-w-[36px] sm:min-w-[44px] flex items-center justify-center flex-shrink-0"
          >
            ✕
          </motion.button>
        </div>
