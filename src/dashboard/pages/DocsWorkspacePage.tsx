    ...
    <div className="flex h-full min-h-[calc(100vh-64px)]">
      ...
      <aside className="hidden md:flex w-56 border-r border-[rgba(139,92,246,0.08)] flex-col bg-[#06061a]/50 shrink-0">
        ...
        <div className="px-2 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between px-3 mb-1">
            <span className="text-xs text-[#B8C4D4] uppercase tracking-wider">Folders</span>
            <button
              onClick={() => setNewFolderOpen(true)}
              className="p-1 rounded hover:bg-white/5 text-[#B8C4D4] hover:text-[#E8E8F0] min-w-[28px] min-h-[28px] flex items-center justify-center"
          ...
          {folders.map(f => (
            ...
            >
              ...
              <span className="text-xs text-[#B8C4D4]/60">{f.doc_count}</span>
            </button>
          ))}
          {folders.length === 0 && (
            <p className="text-xs text-[#B8C4D4]/40 px-3 py-2">No folders yet</p>
          )}
        </div>
      </aside>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs whitespace-nowrap min-h-[44px]
                    ${viewFilter === v.id
                      ...
                      : 'bg-white/5 text-[#B8C4D4] border border-[rgba(139,92,246,0.08)]'
                    }`}
                >
                  {v.label}
                </button>
            ...
            <div className="flex gap-2 max-w-2xl">
              <textarea
                ...
                className="flex-1 bg-transparent border border-[rgba(139,92,246,0.08)] rounded-lg px-3 py-2
                           text-sm text-[#E8E8F0] placeholder-[#B8C4D4]/50 resize-none h-10
          ...
          {loading ? (
            ...
            <BlurFade delay={0.15}>
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-[#E8E8F0] font-medium mb-1">
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          >
                            <Star className={`w-3 h-3 ${doc.pinned ? 'text-[#FFD700] fill-[#FFD700]' : 'text-[#B8C4D4]'}`} />
                          </button>
                          <button
                            ...
                            aria-label="Delete"
                          >
                            <Trash2 className="w-3 h-3 text-[#B8C4D4] hover:text-red-400" />
                          </button>
                        </div>
                      </div>
                      <h3 className="text-[#E8E8F0] font-medium text-sm mb-1 line-clamp-1">
                        {doc.title || 'Untitled'}
                      </h3>
                      <p className="text-[#B8C4D4] text-xs line-clamp-2 mb-3">
                        {doc.content_text?.slice(0, 120) || 'Empty document'}
                      </p>
                      <div className="flex items-center justify-between">
                        <span className="text-[#B8C4D4]/50 text-xs">{formatDate(doc.updated_at)}</span>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] text-[#B8C4D4]/40">{doc.word_count}w</span>
                        </div>
                      </div>
                    </button>
                  </SectionCard>
      ...
      <Dialog open={newFolderOpen} onOpenChange={setNewFolderOpen}>
        <DialogContent className="bg-[rgba(12,12,30,0.6)] backdrop-blur-xl border-[rgba(139,92,246,0.08)] max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-[#E8E8F0]">New Folder</DialogTitle>
          </DialogHeader>
          <Input
            value={newFolderName}
    ...
    <div className="flex flex-col h-full min-h-[calc(100vh-64px)]">
      ...
      <header className="sticky top-0 z-10 flex items-center gap-3 px-4 md:px-6 py-3
        ...
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-white/5 min-w-[44px] min-h-[44px]
          ...
          <ChevronRight className="w-4 h-4 rotate-180 text-[#B8C4D4]" />
        </button>

        <input
          value={title}
          onChange={e => handleTitleChange(e.target.value)}
          className="flex-1 bg-transparent font-semibold text-lg text-[#E8E8F0]
      ...
      <div className="flex items-center gap-1.5 px-3 py-2 border-b border-[rgba(139,92,246,0.08)] bg-[rgba(12,12,30,0.6)] overflow-x-auto scrollbar-hide">
        <span className="text-[10px] text-[#B8C4D4]/60 uppercase tracking-wider mr-1 shrink-0">AI</span>
        {aiActions.map(({ id, icon: Icon, label }) => (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs
                       text-[#B8C4D4] hover:text-[#F59E0B] hover:bg-[#F59E0B]/10
        ...
        <DialogContent className="bg-[rgba(12,12,30,0.6)] backdrop-blur-xl border-[rgba(139,92,246,0.08)] max-w-lg max-h-[80vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 mt-2 min-h-0">
            {convLoading ? (
              ...
              <p className="text-sm text-[#B8C4D4]/60 text-center py-8">No conversations found</p>
            ) : (
              conversations
                ...
                .map((conv) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] text-[#B8C4D4]/50">
                        {new Date(conv.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                    <p className="text-sm text-[#E8E8F0] line-clamp-2">{conv.content}</p>
                    {conv.summary && (
                      <p className="text-xs text-[#B8C4D4] mt-1 line-clamp-1">{conv.summary}</p>
                    )}
                  </button>
                ))
            )}
