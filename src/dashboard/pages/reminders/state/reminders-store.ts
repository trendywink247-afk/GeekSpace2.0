/**
 * reminders-store — Zustand slice for UI-only state in the Reminders page.
 * Keeps all ephemeral state (toasts, modals, inline editing, selection)
 * out of the page component, enabling slim orchestration in RemindersPage.tsx.
 */
...
export const useRemindersUI = create<RemindersUIState>((set) => ({
	...
	recurringEditChoice: null,
	editAsOneOff: false,

	snoozeOpenId: null,
	snoozeCustomId: null,
	snoozeCustomValue: "",
	snoozeHistoryId: null,
	snoozeHistory: [],
	snoozeHistoryLoading: false,
	...
	selectedActiveIds: new Set(),
	selectedIds: new Set(),

	isBulkDeleting: false,
	isBulkRestoringSnooze: false,
	isBatchEditing: false,
	isBulkSnoozing: false,
	isBulkCompleting: false,
	isBulkDeletingActive: false,
	isMarkingAllOverdue: false,
	...
	openComposer: (reminder) =>
		set({ isComposerOpen: true, editingReminder: reminder ?? null }),
	closeComposer: () =>
		set({ isComposerOpen: false, editingReminder: null, editAsOneOff: false }),
	...
	addCompletingId: (id) =>
		set((s) => ({ completingIds: new Set([...s.completingIds, id]) })),
	removeCompletingId: (id) =>
		set((s) => {
			const n = new Set(s.completingIds);
			n.delete(id);
			return { completingIds: n };
		}),
	addJustCompletedId: (id) =>
		set((s) => ({ justCompletedIds: new Set([...s.justCompletedIds, id]) })),
	removeJustCompletedId: (id) =>
		set((s) => {
			const n = new Set(s.justCompletedIds);
			n.delete(id);
			return { justCompletedIds: n };
		}),

	toggleSelectActive: (id) =>
		set((s) => {
			const n = new Set(s.selectedActiveIds);
			n.has(id) ? n.delete(id) : n.add(id);
			return { selectedActiveIds: n };
		}),
