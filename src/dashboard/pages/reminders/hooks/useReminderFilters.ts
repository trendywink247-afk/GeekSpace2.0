/**
 * useReminderFilters — manages all filter/sort/view state with localStorage persistence.
 * Returns both filter values and derived filtered lists.
 */
...
const persist = (updates: Record<string, string>) =>
	localStorage.setItem(
		LS_KEY,
		JSON.stringify({ ...loadFilters(), ...updates }),
	);
...
export function useReminderFilters(reminders: Reminder[]) {
	...
	const [recurrenceFilter, setRecurrenceFilterRaw] = useState<FilterRecurrence>(
		saved.recurrenceFilter ?? "all",
	);
	const [categoryFilter, setCategoryFilterRaw] = useState<FilterCategory>(
		saved.categoryFilter ?? "all",
	);
	const [priorityFilter, setPriorityFilterRaw] = useState<FilterPriority>(
		saved.priorityFilter ?? "all",
	);
	const [searchQuery, setSearchQueryRaw] = useState<string>(
		saved.searchQuery ?? "",
	);
	const [sortMode, setSortModeRaw] = useState<SortMode>(
		saved.sortMode ?? "priority",
	);
	const [groupMode, setGroupMode] = useState<GroupMode>(
		saved.groupMode ?? "date",
	);
	...
	const setRecurrenceFilter = useCallback((v: FilterRecurrence) => {
		setRecurrenceFilterRaw(v);
		persist({ recurrenceFilter: v });
	}, []);
	const setCategoryFilter = useCallback((v: FilterCategory) => {
		setCategoryFilterRaw(v);
		persist({ categoryFilter: v });
	}, []);
	const setPriorityFilter = useCallback((v: FilterPriority) => {
		setPriorityFilterRaw(v);
		persist({ priorityFilter: v });
	}, []);
	const setSearchQuery = useCallback((v: string) => {
		setSearchQueryRaw(v);
		persist({ searchQuery: v });
	}, []);
	...
	const clearFilters = useCallback(() => {
		setFilterRaw("all");
		setRecurrenceFilterRaw("all");
		setCategoryFilterRaw("all");
	...
	const baseFiltered = reminders
		.filter((r) =>
			recurrenceFilter === "recurring"
				? !!r.recurrence
				: recurrenceFilter === "one-off"
					? !r.recurrence
					: true,
		)
		.filter((r) =>
			categoryFilter !== "all" ? r.category === categoryFilter : true,
		)
		.filter((r) =>
			priorityFilter !== "all"
				? (r.priority ?? "normal") === priorityFilter
				: true,
		)
	...
	const activeFiltered = showActive
		? baseFiltered.filter((r) => !r.completed)
		: [];
	const completedFiltered = showCompleted
		? baseFiltered.filter((r) => r.completed)
		: [];
