...
export function useReminderFilters(reminders: Reminder[]) {
  ...
  const setFilter = useCallback((v: FilterStatus) => {
    setFilterRaw(v); persist({ filter: v });
  }, []);
  const setRecurrenceFilter = useCallback((v: FilterRecurrence) => {
    setRecurrenceFilterRaw(v); persist({ recurrenceFilter: v });
  }, []);
  const setCategoryFilter = useCallback((v: FilterCategory) => {
    setCategoryFilterRaw(v); persist({ categoryFilter: v });
  }, []);
  const setPriorityFilter = useCallback((v: FilterPriority) => {
    setPriorityFilterRaw(v); persist({ priorityFilter: v });
  }, []);
  const setSearchQuery = useCallback((v: string) => {
    setSearchQueryRaw(v); persist({ searchQuery: v });
  }, []);
  const setSortMode = useCallback((v: SortMode) => {
    setSortModeRaw(v); persist({ sortMode: v });
  }, []);
  ...
  const hasActiveFilters  =
    ...
    priorityFilter !== 'all' || recurrenceFilter !== 'all' || searchQuery !== '';

  return {
    filter, recurrenceFilter, categoryFilter, priorityFilter,
    searchQuery, sortMode, groupMode, viewMode,
    setFilter, setRecurrenceFilter, setCategoryFilter, setPriorityFilter,
    setSearchQuery, setSortMode, setGroupMode, setViewMode,
    clearFilters,
