...
import type { FilterStatus, FilterRecurrence, FilterCategory, FilterPriority, SortMode, GroupMode, ViewMode, StatsBarProps } from '../types';
  ...
  return (
    <div className="space-y-3">
      ...
      <BlurFade delay={0.18} inView>
        <div className="rounded-2xl p-3 backdrop-blur-xl space-y-2.5"
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <Input placeholder="Search reminders…" value={searchQuery} onChange={e => onSearchQueryChange(e.target.value)}
            ...
            <div className="flex items-center rounded-xl overflow-hidden" style={{ border: '1px solid var(--ag-border-subtle)' }}>
              {(['list', 'calendar'] as ViewMode[]).map((mode, i) => {
                ...
                return (
                  <motion.button key={mode} whileTap={{ scale: 0.92 }} onClick={() => onViewModeChange(mode)}
                    className="flex items-center justify-center w-11 h-11 transition-colors"
                    style={{ background: viewMode === mode ? 'rgba(139,92,246,0.12)' : 'transparent', color: viewMode === mode ? 'var(--ag-violet-soft)' : 'var(--ag-text-muted)', borderLeft: i > 0 ? '1px solid var(--ag-border-subtle)' : 'none' }}
            ...
            <div className="hidden sm:flex items-center rounded-xl overflow-hidden" style={{ border: '1px solid var(--ag-border-subtle)' }}>
              {[{ label: 'CSV', onClick: onExportCsv }, { label: 'ICS', onClick: onExportIcs }].map(({ label, onClick }, i) => (
                <motion.button key={label} whileTap={{ scale: 0.92 }} onClick={onClick}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <span className="w-px self-stretch mx-0.5" style={{ background: 'var(--ag-border-subtle)' }} />
            ...
            {(['date', 'category'] as GroupMode[]).map(m => (
              <PillButton key={m} active={groupMode === m} accent="violet" onClick={() => onGroupModeChange(m)}>
                <LayoutGrid className="w-3 h-3" />{m === 'date' ? 'By date' : 'By category'}
              </PillButton>
            ))}
          </div>
          {hasActiveFilters && (
