    ...
    <div className="space-y-6 w-full max-w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Facts your agent remembers about you <Badge className="ml-1">{memories.length}</Badge></p>
        </div>
        <Button variant="outline" onClick={load} aria-label="Refresh"><RefreshCw className="w-4 h-4" /></Button>
      </div>
      {apiErr && (
      ...
      <Card>
        <CardContent className="p-4">
          <p className="text-xs text-gray-500 mb-3 font-medium uppercase tracking-wider">Add memory</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 max-w-sm mx-auto">Add facts above, or your agent will extract them as you chat.</p>
        </div>
      ) : (
              ...
              <tbody>
                {memories.map(m => (
                  <tr key={m.id} className="border-b hover:bg-gray-900/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {editId===m.id ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => void doSave(m)} disabled={saving} className="p-1.5 rounded min-w-[32px] min-h-[32px] flex items-center justify-center bg-green-900/30 text-green-400" aria-label="Save"><Check className="w-4 h-4" /></button>
                          <button onClick={() => setEditId(null)} className="p-1.5 rounded min-w-[32px] min-h-[32px] flex items-center justify-center bg-gray-800" aria-label="Cancel"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs hidden md:table-cell">{formatTs(m.last_used)}</td>
                    <td className="px-4 py-3"><div className="flex items-center gap-1 justify-end">
                      <button onClick={() => {setEditId(m.id);setEditVal(m.value);}} className="p-2 rounded min-w-[36px] min-h-[36px] flex items-center justify-center text-gray-500 hover:text-cyan-400" aria-label="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => void doDel(m.id)} className="p-2 rounded min-w-[36px] min-h-[36px] flex items-center justify-center text-gray-500 hover:text-red-400" aria-label="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
