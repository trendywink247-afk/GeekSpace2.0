...
function srcBadge(s: string): string {
  if(s==="manual") return "bg-[#00F0FF]/10 text-[#00F0FF]";
    ...
    <div className="space-y-6 w-full max-w-full">
      ...
      <Card className="border-[#00F0FF]/10">
        <CardContent className="p-4">
          <p className="text-xs text-[#8892A4] mb-3 font-medium uppercase tracking-wider">Add memory</p>
          <div className="flex flex-col sm:flex-row gap-2">
            <Input placeholder="Key (e.g. preferred_name)" value={addKey} onChange={e => setAddKey(e.target.value)} className="flex-1 bg-[#0C0C18] border-[#00F0FF]/10 focus:border-[#00F0FF] focus-visible:ring-[#00F0FF]/30 placeholder:text-[#8892A4]/50" onKeyDown={e => {if(e.key==="Enter") void doAdd();}} />
            <Input placeholder="Value" value={addVal} onChange={e => setAddVal(e.target.value)} className="flex-1 bg-[#0C0C18] border-[#00F0FF]/10 focus:border-[#00F0FF] focus-visible:ring-[#00F0FF]/30 placeholder:text-[#8892A4]/50" onKeyDown={e => {if(e.key==="Enter") void doAdd();}} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-medium mb-2 text-[#F4F6FF]">No memories yet</h3>
          <p className="text-[#8892A4] max-w-sm mx-auto">Chat with Weebo and say &ldquo;remember that...&rdquo; to get started.</p>
        </div>
      ) : (
        <Card className="overflow-hidden border-[#00F0FF]/10">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-[#0C0C18] text-[#8892A4] uppercase text-xs tracking-wider">
              ...
              <tbody>
                {memories.map(m => (
                  <tr key={m.id} className="border-b border-[#00F0FF]/5 hover:bg-[#00F0FF]/5 transition-colors">
                    <td className="px-4 py-3 font-mono text-[#00F0FF] text-xs">{m.key}</td>
                    <td className="px-4 py-3 text-[#F4F6FF]">
                      {editId===m.id ? (
                        <div className="flex items-center gap-2">
                          <Input value={editVal} onChange={e => setEditVal(e.target.value)} className="h-8 text-sm bg-[#0C0C18] border-[#00F0FF]/10 focus:border-[#00F0FF] focus-visible:ring-[#00F0FF]/30" autoFocus onKeyDown={e => {if(e.key==="Enter")void doSave(m);if(e.key==="Escape")setEditId(null);}} />
                    ...
                    <td className="px-4 py-3 text-[#8892A4] text-xs hidden md:table-cell">{formatTs(m.last_used)}</td>
                    <td className="px-4 py-3"><div className="flex items-center gap-1 justify-end">
                      <button onClick={() => {setEditId(m.id);setEditVal(m.value);}} className="p-2 rounded min-w-[44px] min-h-[44px] flex items-center justify-center text-[#8892A4] hover:text-[#00F0FF] focus-visible:ring-2 focus-visible:ring-[#00F0FF]/50" aria-label="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => void doDel(m.id)} className="p-2 rounded min-w-[44px] min-h-[44px] flex items-center justify-center text-[#8892A4] hover:text-[#FF2D78] focus-visible:ring-2 focus-visible:ring-[#00F0FF]/50" aria-label="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
