...
const TYPE_ACCENT: Record<string, string> = { reminder: 'var(--ag-violet-soft)', habit: 'var(--ag-emerald)', custom: 'var(--ag-amber)' };
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-col gap-1.5">
        {blocks.map((block, i) => {
          ...
          return (
            <div key={block.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
              style={{ background: 'var(--ag-active-bg)', border: '1px solid var(--ag-border-subtle)' }}>
              <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                style={{ background: `color-mix(in srgb, ${accent} 12%, transparent)`, color: accent }}>{i + 1}</span>
