...
interface BlockComposerProps {
  inputRef: RefObject<HTMLInputElement | null>; title: string; duration: number;
  onTitleChange: (v: string) => void; onDurationChange: (v: number) => void;
  ...
  return (
    <div className="rounded-xl p-3 space-y-2.5 animate-in fade-in slide-in-from-top-1 duration-200"
      ...
      <div className="flex items-center gap-2">
        <Input ref={inputRef} value={title} onChange={e => onTitleChange(e.target.value)} placeholder="What are you working on?"
          className="h-9 sm:h-8 text-xs flex-1"
        ...
        <Button size="sm" className="h-9 sm:h-8 px-3 text-xs font-medium min-w-[44px]"
          ...
          onClick={onSubmit} disabled={!title.trim()}>Add</Button>
        <button onClick={onCancel} className="w-11 h-11 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: 'var(--ag-text-muted)' }}>
