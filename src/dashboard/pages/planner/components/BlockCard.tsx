...
export function TimeBlockCard({ block, onRemove }: { block: TimeBlock; onRemove: (id: string) => void }) {
  ...
  const durationLabel = block.duration < 1 ? `${Math.round(block.duration * 60)}m` : block.duration === 1 ? '1h' : `${block.duration}h`;
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${block.color}1A` }}>
            {block.type === 'reminder' ? <Bell className="w-3 h-3" style={{ color: accent }} /> :
             block.type === 'habit' ? <Flame className="w-3 h-3" style={{ color: accent }} /> :
             <LayoutGrid className="w-3 h-3" style={{ color: accent }} />}
          </div>
          <span className="text-xs font-medium truncate" style={{ color: 'var(--ag-text-primary)' }}>{block.title}</span>
        </div>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className="flex items-center gap-0.5 text-[10px] tabular-nums" style={{ color: 'var(--ag-text-muted)' }}>
            <Clock className="w-3 h-3" />{durationLabel}
          </span>
          <button onClick={() => onRemove(block.id)}
            className="w-11 h-11 sm:w-6 sm:h-6 rounded flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100 hover:text-rose-500 hover:bg-rose-500/10"
            style={{ color: 'var(--ag-text-muted)' }} title="Remove block">
...
export function BacklogCard({ item, onDragStart }: { item: BacklogItem; onDragStart: (item: BacklogItem) => void }) {
  const borderColor = item.type === 'reminder' ? 'var(--ag-violet-soft)' : 'var(--ag-emerald)';
  ...
  return (
    <div draggable onDragStart={() => onDragStart(item)}
      className="group flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-grab active:cursor-grabbing active:scale-[0.96] select-none min-h-[44px] transition-all duration-150"
      ...
      <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
        style={{ background: item.type === 'reminder' ? 'rgba(167,139,250,0.12)' : 'rgba(16,185,129,0.12)' }}>
        {item.icon === 'bell' ? <Bell className="w-3.5 h-3.5" style={{ color: borderColor }} /> : <Flame className="w-3.5 h-3.5" style={{ color: borderColor }} />}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-xs truncate leading-relaxed" style={{ color: 'var(--ag-text-primary)' }}>{item.title}</p>
      </div>
      {priorityColor && item.priority && item.priority !== 'normal' && (
        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded uppercase tracking-wider flex-shrink-0"
          style={{ color: priorityColor, backgroundColor: `${priorityColor}18`, border: `1px solid ${priorityColor}30` }}>
          {item.priority}
        </span>
      )}
    </div>
