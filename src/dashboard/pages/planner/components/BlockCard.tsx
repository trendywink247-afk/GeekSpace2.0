// ============================================================
// BlockCard — Aurora-styled time block (placed on timeline)
//             and BacklogCard (drag source from backlog panel).
// ============================================================
...
// ── Aurora accent map for block types ─────────────────────────────────────
...
export function TimeBlockCard({ block, onRemove }: TimeBlockCardProps) {
	...
	return (
		<div
			className="group relative rounded-xl px-3 py-2.5 mb-1.5
        transition-all duration-150 hover:scale-[1.01] active:scale-[0.97]"
			style={{
				backgroundColor: `${block.color}0D`,
				border: `1px solid ${block.color}30`,
				borderLeft: `3px solid ${block.color}`,
				boxShadow: `0 2px 8px ${block.color}0A`,
			}}
		>
			<div className="flex items-center justify-between gap-2">
				...
				<div className="flex items-center gap-1.5 flex-shrink-0">
					...
					<button
						...
						className="w-11 h-11 sm:w-6 sm:h-6 rounded flex items-center justify-center
...
export function BacklogCard({ item, onDragStart }: BacklogCardProps) {
	const borderColor =
	...
	return (
		<div
			...
			className="group flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-grab active:cursor-grabbing active:scale-[0.96]
        select-none min-h-[44px] transition-all duration-150"
			style={{
			...
			onMouseEnter={(e) => {
				(e.currentTarget as HTMLDivElement).style.boxShadow =
					"0 4px 12px rgba(0,0,0,0.28), 0 0 0 1px rgba(139,92,246,0.18)";
			}}
			onMouseLeave={(e) => {
				(e.currentTarget as HTMLDivElement).style.boxShadow =
