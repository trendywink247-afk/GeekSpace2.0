...
export function usePlannerView({ currentDate, todayBlocks, habits, reminders }: UsePlannerViewOptions) {
  const now = new Date();
  ...
  const backlogItems = useMemo<BacklogItem[]>(() => {
    ...
    reminders
      .filter((r: Reminder) => {
        ...
        if (!r.datetime) return isSameDay(currentDate, n);
  ...
  const topThree = useMemo(() =>
    todayBlocks.slice().sort((a, b) => a.startHour - b.startHour).slice(0, 3),
    [todayBlocks]
  );
