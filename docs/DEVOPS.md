...
- Inputs:
  - `action` — closed choice list. Current whitelist:
    ...
    - `rotate-jwt-encryption` — real rotation. Delegates to `ops/rotate-app-secrets.sh --re-encrypt` on the VPS. Generates new `JWT_SECRET` + `ENCRYPTION_KEY`, runs `server/scripts/reencrypt-api-keys.ts` against the live DB volume to re-key every `api_keys.key_encrypted` row, atomically writes `/root/.agentin-secrets` via `install -m 600`, restarts the `geekspace` container, and smoke-checks `/api/health`. Refuses to run if the VPS `/root/GeekSpace2.0` working tree is dirty. Requires board approval of the maintenance window before dispatch. See `docs/SSH-ACCESS.md` § `ENCRYPTION_KEY rotation`.
    - `rotate-jwt-encryption-rollback` — restores the most recent backup under `/root/secret-backups/agentin-secrets.*.bak` over `/root/.agentin-secrets` and restarts the container. Used only if a post-rotation smoke test fails. Does **not** reverse the `api_keys` re-encryption; that requires a second `rotate-jwt-encryption`-equivalent run with swapped keys.
