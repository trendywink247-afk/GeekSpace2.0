...
**Unattributed keys** (present in `authorized_keys` but no known owner) must be moved to `quarantined` status and removed within one business day of discovery. Removal uses the `remove-ssh-key` arm of [`.github/workflows/ops-remote-exec.yml`](../.github/workflows/ops-remote-exec.yml) — dispatch it with the key's SHA256 fingerprint (preferred) or exact key comment as the `key_identifier` input. The arm refuses to run on no-match or ambiguous matches, writes a timestamped backup of `authorized_keys` before mutation, and prints a before/after diff with the key blob redacted for the audit log. The `production-ops` environment gate still requires board approval on every dispatch. Never ad-hoc SSH.
