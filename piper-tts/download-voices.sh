...
curl -L -o "$VOICE_DIR/en_US-amy-medium.onnx" \
  "$BASE_URL/voice-en_US-amy-medium.onnx" || echo "Warning: Amy voice download failed"
...
curl -L -o "$VOICE_DIR/en_US-ryan-medium.onnx" \
  "$BASE_URL/voice-en_US-ryan-medium.onnx" || echo "Warning: Ryan voice download failed"
