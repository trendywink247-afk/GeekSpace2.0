...
import { type ChatMessage, type Provider, routeChat } from "./llm.js";
...
export interface ReactLoopOptions {
	...
	/** User subscription plan slug — forwarded to routeChat for tier-based provider selection */
	userPlan?: string;
...
export async function runReactLoop(
	...
	for (let i = 0; i < MAX_REACT_ITERATIONS; i++) {
		...
		opts.onStep?.({
			type: "thinking",
			content:
				i === 0 ? "Analyzing your request..." : "Reasoning about results...",
			iteration: i,
		});
		if (opts.agentId)
			emitThinking(
				opts.userId,
				opts.agentId,
				i === 0 ? "Analyzing request..." : "Reasoning about results...",
			);
		...
		if (actions.length === 0) {
			...
			if (opts.agentId)
				emitResponding(opts.userId, opts.agentId, "Writing response...");
			break;
		}
		...
		for (const action of actions) {
			...
			if (
				opts.onConfirmNeeded &&
				needsConfirmationForUser(opts.userId, action.tool, action.params)
			) {
				opts.onStep?.({
					type: "tool_call",
					content: `Requesting confirmation for ${action.tool}...`,
					tool: action.tool,
					iteration: i + 1,
				});
				const confirmation = await opts.onConfirmNeeded(
					action.tool,
					action.params,
				);
				if (!confirmation.approved) {
					...
					observations.push(
						`Tool ${action.tool} was REJECTED by user: ${reason}. Do not retry this action. Adjust your approach.`,
					);
					opts.onStep?.({
						type: "tool_result",
						content: `${action.tool} rejected: ${reason}`,
						tool: action.tool,
						iteration: i + 1,
					});
					continue;
			...
			const toolDesc =
				action.tool === "web_search"
					? `Searching: "${action.params?.query || ""}"`
					: action.tool === "crawl_url"
						? `Reading: ${action.params?.url || ""}`
						: action.tool === "take_screenshot"
							? `Screenshotting: ${action.params?.url || ""}`
							: action.tool === "generate_image"
								? `Generating image: "${action.params?.prompt || ""}"`
								: `Running ${action.tool}...`;
			opts.onStep?.({
				type: "tool_call",
				content: toolDesc,
			...
			const resultSummary = actionResult.success
				? actionResult.data?.summary
					? String(actionResult.data.summary).slice(0, 200)
					: actionResult.message.slice(0, 200)
			...
			if (opts.agentId)
				emitToolResult(opts.userId, opts.agentId, action.tool, resultSummary);
		...
		if (i === MAX_REACT_ITERATIONS - 1) {
			opts.onStep?.({
				type: "drafting",
				content: "Writing final response...",
				iteration: i,
			});
			if (opts.agentId)
				emitResponding(opts.userId, opts.agentId, "Writing response...");
