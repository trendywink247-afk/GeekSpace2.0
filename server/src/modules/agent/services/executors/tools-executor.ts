/**
 * Tools & data executor — reminders, notes, habits, focus, expenses, email,
 * telegram, automation, calendar, inbox, memory, goals, escalation.
 */
...
export async function executeToolsAction(
  userId: string,
  tool: string,
  ...
  switch (tool) {
      ...
      case 'run_workflow': {
        ...
        for (const step of steps.slice(0, 5)) {
          try {
            const { executeAction: execAction } = await import('../action-executor.js');
            const stepResult = await execAction(userId, { tool: step.action, params: step.params || {} });
