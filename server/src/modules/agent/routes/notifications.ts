...
router.get('/notifications', requireAuth, (req, res) => {
  ...
  const notifications = getNotifications(authReq.user!.id, unreadOnly, limit);
  const unreadCount = getUnreadCount(authReq.user!.id);
...
router.patch('/notifications/:id/read', requireAuth, (req, res) => {
  const authReq = req as AuthRequest;
  const success = markAsRead(req.params.id, authReq.user!.id);
  res.json({ success });
});
...
router.post('/notifications/read-all', requireAuth, (req, res) => {
  const authReq = req as AuthRequest;
  const count = markAllAsRead(authReq.user!.id);
...
router.delete('/notifications/:id', requireAuth, (req, res) => {
  const authReq = req as AuthRequest;
  const success = deleteNotification(req.params.id, authReq.user!.id);
  res.json({ success });
});
