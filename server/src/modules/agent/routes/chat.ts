...
import {
	formatContextBlock,
	loadPicoContext,
} from "../../../services/pico-context.js";
import {
	type BridgeRequest,
	bridgeChat,
} from "../../../services/pico-kimi-bridge.js";
import {
	formatReceiptCompact,
	type ReceiptItem,
...
import { type AgentRole, getAgentRoles } from "../services/agent-registry.js";
import { emitDone, emitThinking } from "../services/agent-state-bus.js";
import { filterToolsBlock } from "../services/agent-tools-map.js";
import {
	autoTitleConversation,
	getOrCreateConversation,
	updateConversationMeta,
} from "../services/conversation-threads.js";
...
import {
	...
	generateHypotheses,
	isAmbiguous,
} from "../services/hypothesis-service.js";
import {
	type ChatMessage,
	classifyIntent,
	computeCreditCost,
...
async function incrementRateLimitTracker(userId: number): Promise<void> {
	...
	try {
		...
		/* Redis fail = degrade to unlimited */
	}
}
 ...
 */
async function _getRateLimitStatus(
	userId: number,
): Promise<{ remaining: number; limit: number; windowMinutes: number }> {
	const key = `chat:rl:${userId}`;
	try {
		const raw = await cacheGet(key);
		const count = raw ? parseInt(raw, 10) : 0;
		return {
			remaining: Math.max(0, RL_LIMIT - count),
			limit: RL_LIMIT,
			windowMinutes: 15,
		};
	} catch {
...
router.post("/generate-content", requireAuth, async (req: AuthRequest, res) => {
	try {
		const { type, tags, name } = req.body as {
			type: string;
			tags: string[];
			name?: string;
		};
		if (!type || !tags || tags.length === 0) {
			return res
				.status(400)
				.json({ error: "type and at least 1 tag required" });
		}
...
Skills/interests: ${Array.isArray(tags) ? tags.join(", ") : tags || "software development"}`,
		...
		if (!systemPrompt) {
			return res.status(400).json({
				error:
					"type must be headline, bio, about, skills, bio-batch, or portfolio-batch",
			});
		}
		...
		const sub = db
			.prepare(
				"SELECT credits_remaining, billing_cycle_end FROM subscriptions WHERE user_id = ?",
			)
			.get(req.userId!) as
			| { credits_remaining: number; billing_cycle_end: string }
			| undefined;
...
- accent must be a single hex color that works as UI accent`;
		...
		try {
			...
			if (!parsed.gradient || !parsed.name || !parsed.accent)
				throw new Error("Invalid response");
			const creditCost = computeCreditCost(
				"edith",
				edithResult.tokensIn,
				edithResult.tokensOut,
			);
			...
			res.json({
				gradient:
					"linear-gradient(135deg, #1a0533 0%, #0a0a1a 40%, #001a33 100%)",
				name: "Deep Space",
				accent: "#7B61FF",
			});
		}
...
router.post(
	...
	async (req: AuthRequest, res) => {
		...
		const reqConversationId = (req.body as { conversationId?: string })
			.conversationId;
		...
		const reqExistingArtifactId = (req.body as { existingArtifactId?: string })
			.existingArtifactId;
		...
		if (isGuestUser) {
			const portfolioUsername =
				req.portfolioUsername ||
				(req.body as { portfolioUsername?: string }).portfolioUsername;
			if (!portfolioUsername) {
				res.status(403).json({
					error: "Please use the portfolio chat endpoint for visitor access.",
				});
				return;
			}
			...
			try {
				const ownerRow = db
					.prepare(
						"SELECT id, name, role, company FROM users WHERE username = ?",
					)
					.get(portfolioUsername) as Record<string, unknown> | undefined;
				if (!ownerRow) {
				...
				const portfolioRow = db
					.prepare(
						"SELECT about, skills, projects FROM portfolios WHERE user_id = ?",
					)
					.get(ownerRow.id as string) as Record<string, unknown> | undefined;
				const agentCfg = db
					.prepare("SELECT * FROM agent_configs WHERE user_id = ?")
					.get(ownerRow.id as string) as Record<string, unknown> | undefined;
				const skills: string[] = JSON.parse(
					(portfolioRow?.skills as string) || "[]",
				);
				...
				res.json({
					reply: `Hi! I'm the AI assistant for ${portfolioUsername}'s portfolio. How can I help you?`,
					agentName: "Jarvis",
					ownerName: portfolioUsername,
				});
			}
			return;
		}
		...
		try {
			const agentConfig = db
				.prepare("SELECT * FROM agent_configs WHERE user_id = ?")
				.get(userId) as Record<string, unknown> | undefined;
			const user = db
				.prepare("SELECT name, credits FROM users WHERE id = ?")
				.get(userId) as Record<string, unknown> | undefined;
			...
			if (files && files.length > 0) {
				try {
					const processed = await Promise.all(
						files.map((f) => processUploadedFile(f)),
					);
			...
			const sub = db
				.prepare(
					"SELECT plan, credits_remaining, billing_cycle_end FROM subscriptions WHERE user_id = ?",
				)
				.get(userId) as
				| { plan: string; credits_remaining: number; billing_cycle_end: string }
				| undefined;
...
You are assisting via the Agentin terminal. Be concise. No markdown headers. Plain text or simple code blocks only.`;
				...
				const terminalResult = await routeChat(terminalMessages, {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 'terminal', 'ai.chat')`,
				).run(
					uuid(),
					userId,
					terminalResult.provider,
					terminalResult.model,
					terminalResult.tokensIn,
					terminalResult.tokensOut,
					terminalResult.creditCost,
				);
				logConversation(
					userId,
					"assistant",
				...
				res.json({
					text: terminalResult.reply,
					provider: "jarvis-terminal",
					creditCost: terminalResult.creditCost,
					latencyMs: terminalResult.latencyMs,
				});
				return;
			}
			...
			if (message.startsWith("/premium ") || message.startsWith("/edith ")) {
				...
				try {
					if (sub && sub.credits_remaining < 10) {
						res.json({
							text: "Not enough credits for task planning (minimum 10 required).",
							route: "error",
							provider: "builtin",
							latencyMs: 0,
						});
					...
					const { planTasks: planTasksFn, queueTasks: queueTasksFn } =
						await import("../services/pico-fleet.js");
					const start = Date.now();
					const { tasks, creditCost } = await planTasksFn(
						userId,
						taskDesc,
						userPlan,
					);
					...
					if (tasks.length === 0) {
						res.json({
							text: `No actionable tasks could be planned. Credits used: ${creditCost}`,
							route: "pico-fleet",
							provider: "edith",
							latencyMs,
					...
					const summary = tasks
						.map(
							(t) => `[slot ${t.agent_slot}] ${t.task_type}: ${t.description}`,
						)
						.join("\n");
					...
					res.json({
						text: `Task planning failed: ${err instanceof Error ? err.message : "Unknown error"}`,
						route: "error",
						provider: "builtin",
						latencyMs: 0,
					});
					...
					try {
						...
						if (tasks.length > 0) {
							...
							const summary = tasks
								.map(
									(t) =>
										`**${t.task_type.replace(/_/g, " ")}**: ${t.description}`,
								)
								.join("\n");
							...
							logConversation(
								userId,
								"assistant",
								`Done! I've queued ${taskIds.length} task(s) for Weebo:\n${summary}`,
							);
			...
			if (!forceRoute) {
				const createWebsitePattern =
					/\b(?:build|create|make|generate)\b.{0,80}\b(?:website|site|portfolio|landing|blog|page)\b/i;
				const editWebsitePattern =
					/\b(?:change|update|edit|modify|redesign|redo|refresh|revamp|adjust|tweak|rebuild)\b.{0,80}\b(?:website|site|portfolio|landing|blog|page|theme|background|color)\b/i;
				if (
					createWebsitePattern.test(message) ||
					editWebsitePattern.test(message)
				) {
					const { executeAction } = await import(
						"../services/action-executor.js"
					);
					const msgLower = message.toLowerCase();
					const nameMatch =
						message.match(
							/\b(?:for|name is)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b/,
						) ??
						message.match(/\bmy name is\s+([A-Za-z]+(?:\s+[A-Za-z]+)?)\b/i);
					const themeMatch = msgLower.match(
						/\b(dark|light|purple|blue|gradient)\b/,
					);
					const templateMatch = msgLower.match(/\b(landing|blog|business)\b/);
					const locationMatch = message.match(
						/\bfrom\s+([A-Z][a-zA-Z\s]{2,20})\b/,
					);
					const professionMatch = message.match(
						/\b(developer|designer|engineer|writer|photographer|artist|consultant|manager|teacher|doctor|lawyer|freelancer)\b/i,
					);
					const isEdit =
						editWebsitePattern.test(message) &&
						!createWebsitePattern.test(message);
					...
					if (isEdit) {
						if (!editTargetId) {
							...
							if (latest) {
								...
								try {
									editTargetIsTemplate = !!JSON.parse(latest.metadata || "{}")
										.template;
								} catch {
									/* ignore */
								}
							...
							if (existing?.metadata) {
								try {
									editTargetIsTemplate = !!JSON.parse(existing.metadata)
										.template;
								} catch {
									/* ignore */
								}
					...
					const isPersonalTemplate =
						...
						(!isEdit &&
							(!!templateMatch ||
								...
								!!professionMatch ||
								!!locationMatch ||
								/\b(my (portfolio|blog|website|site|page|landing)|portfolio (website|site|page))\b/i.test(
									message,
								)));
					...
					const fastResult = await executeAction(userId, {
						tool: "generate_code",
						params: artifactParams,
					});
					if (fastResult.success) {
						const isUpdated =
							isEdit && !!(fastResult.data as Record<string, unknown>)?.updated;
						const replyText = isUpdated
							...
							actionResults: [
								{
									tool: "generate_code",
									success: true,
									previewUrl: fastResult.previewUrl,
									artifactId: fastResult.artifactId,
								},
							],
						});
						return;
			...
			if (!forceRoute) {
				const imageVerbNounPattern =
					/\b(?:generate|create|make|render|produce|show me|i want|give me|can you make|imagine|visualize)\b.{0,60}\b(?:image|picture|photo|illustration|artwork|art|painting|portrait|wallpaper|sketch)\b/i;
				const drawingVerbPattern =
					/\b(?:draw|paint|sketch|imagine|visualize)\b\s+\S/i;
				const isReminderMsg = /\b(?:remind|reminder|schedule|alarm)\b/i.test(
					message,
				);
				if (
					!isReminderMsg &&
					(imageVerbNounPattern.test(message) ||
						drawingVerbPattern.test(message))
				) {
					const promptMatch =
						message.match(
							/\b(?:generate|create|make|draw|render|produce|show me|i want|give me|can you make|can you draw|paint|sketch|imagine|visualize)\b.{0,20}\b(?:image|picture|photo|illustration|artwork|art|drawing|painting|portrait|wallpaper|sketch)\b(?:\s+of\s+|\s+showing\s+|\s+with\s+|\s+)?([\s\S]+)/i,
						) ??
						message.match(
							/\b(?:draw|paint|sketch|imagine|visualize)\b\s+(?:me\s+)?(?:a\s+|an\s+|the\s+)?([\s\S]+)/i,
						);
					const rawPrompt = promptMatch?.[1]?.trim() || message;
					const imagePrompt =
						rawPrompt.replace(/^(?:a\s+|an\s+|the\s+|me\s+)/i, "").trim() ||
						message;
					try {
						const imgFastResult = await executeAction(userId, {
							tool: "generate_image",
								...
								actionResults: [
									{
										tool: "generate_image",
										success: true,
										imageUrl: imgFastResult.imageUrl,
										imageId: imgFastResult.imageId,
										prompt: imagePrompt,
									},
								],
							});
						...
						logger.warn(
							...
							"Image generation fast-path failed, falling through to LLM",
						);
					}
				}
			}
			...
			const BARE_DOMAIN_RE =
				/\b([a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)+\.[a-zA-Z]{2,})\b/;
			const hasUrl =
				/https?:\/\/\S+/.test(message) || BARE_DOMAIN_RE.test(message);
			...
			const msgHasNonLatin =
				/\p{Script=Devanagari}|\p{Script=Telugu}|\p{Script=Arabic}|\p{Script=Tamil}|\p{Script=Gujarati}/u.test(
					message,
				);
			const HINGLISH_SET = new Set([
				"aap",
				"kya",
				"kaise",
				"hai",
			...
			const msgWords = message
				...
				.split(/\s+/);
			const msgIsHinglish =
				!forceRoute && msgWords.filter((w) => HINGLISH_SET.has(w)).length >= 2;
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, 'web', 'ai.chat')`,
					...
					const { text: cleanEdithReply, actions: edithActions } = parseActions(
						edithResult.text,
					);
					...
					logConversation(
						userId,
						"assistant",
						cleanEdithReply || edithResult.text,
						"edith",
						config.moonshotReasoningModel,
					);
					...
					logger.warn(
						{ err, userId },
						"Premium (Moonshot) call failed, falling back to local",
					);
			...
			if (forceRoute === "bridge") {
				try {
					...
					if (!forceAgent) {
						...
						if (delegation) {
							...
							delegationInfo = {
								...
								delegationLimit: delegation.delegationLimit,
								remaining: delegation.remaining,
							};
							// Emit visible delegation event in Office timeline + canvas
							const currentAgent =
								(agentConfig?.personality as string) || "weebo";
							emitDelegation(
								userId,
								currentAgent,
								delegation.agent,
							...
							emitCommSent(
								...
								currentAgent,
								delegation.agent,
								`Handling "${message.slice(0, 60)}…" — ${delegation.reason}`,
							);
						}
					}
					...
					// Emit 'thinking' so office canvas visibly reacts during the (often slow) bridge call
					const _canvasAgent = (delegatedAgent ?? (agentConfig?.personality as string) ?? "weebo") as string;
					emitThinking(String(userId), _canvasAgent, `Thinking: ${message.slice(0, 48)}`);
					...
					if (delegatedAgent && delegationInfo) {
						...
						emitCommReceived(
							...
							delegatedAgent,
							currentAgent,
							`Done: ${bridgeResult.text.slice(0, 60)}`,
						);
					}
					...
					db.prepare(
						`INSERT INTO usage_events (id, user_id, provider, model, tokens_in, tokens_out, cost_usd, channel, tool)
          VALUES (?, ?, ?, ?, ?, ?, ?, 'web', 'ai.bridge')`,
					).run(
						uuid(),
						userId,
						bridgeResult.provider,
						bridgeResult.model,
						bridgeResult.tokensIn,
						bridgeResult.tokensOut,
						bridgeResult.creditCost,
					);
					...
					if (bridgeResult.creditCost > 0) {
						db.prepare(
							"UPDATE users SET credits = MAX(0, credits - ?) WHERE id = ?",
						).run(bridgeResult.creditCost, userId);
					...
					// Parse and execute any tool actions from LLM response
					const { text: cleanReply, actions: parsedActions } = parseActions(
						bridgeResult.text,
					);
					...
					extractMemoriesWithAI(
						...
						logger.debug({ err: e }, "background task failed"),
					);
					...
					logger.warn(
						{ err, userId },
						"Bridge route failed, falling back to standard router",
					);
					...
					if (delegationInfo) {
						...
						logger.info(
							{ userId },
							"Rolled back delegation counter after bridge failure",
						);
					}
			...
			checkKeywordTriggers(userId, message).catch((e: unknown) =>
				logger.debug({ err: e }, "background task failed"),
			);
			...
			if (!forceAgent) {
				if (bridgeFellThrough) {
					...
					const { detectDelegationTarget, canDelegate: checkCan } =
						await import("../../../services/delegation.js");
					...
					if (target) {
						...
						if (status.allowed) {
							...
							localDelegation = {
								...
								delegationCount: status.used,
								delegationLimit: status.limit,
					...
					if (localDelegation) {
						...
						emitDelegation(
							userId,
							currentAgent,
							localDelegation.agent,
							localDelegation.reason,
						);
						emitCommSent(
							userId,
							currentAgent,
							localDelegation.agent,
							`Handling "${message.slice(0, 60)}…" — ${localDelegation.reason}`,
						);
					}
				}
				...
				if (localDelegation) {
					...
					effectiveAgentConfig = {
						...(agentConfig || {}),
						personality: localDelegation.agent,
						name: delegatedPersonality.name,
					};
					...
					emitThinking(
						userId,
						localDelegation.agent,
						`${delegatedPersonality.name} is processing...`,
					);
				}
			}
			...
			if (hasUrl) {
				...
				if (targetUrl) {
					try {
						...
						logger.info(
							{ url: targetUrl, chars: pageContent.length },
							"web_research: pre-fetched URL for LLM context",
						);
					} catch (fetchErr) {
						logger.warn(
							{ url: targetUrl, err: (fetchErr as Error).message },
							"web_research: pre-fetch failed, proceeding without content",
						);
					}
				}
			}
			...
			if (forceRoute === "local") {
				...
				logger.info(
					{ userId, reason: msgHasNonLatin ? "non-latin-script" : "hinglish" },
					"web chat multilingual — routing to Groq",
				);
				...
				resolvedProvider = "ollama";
				logger.info(
					{ userId, reason: "local_preference_direct" },
					"Skipping pickProvider — direct Ollama",
				);
			} else {
			...
			if (isAmbiguous(message)) {
				try {
					const recentText = history
						...
						.join("\n");
					const hypResult = await generateHypotheses(
						userId,
						message,
						recentText,
					);
			...
			const useDeepReasoning =
				complexity === "multi_step" || complexity === "multi_agent";
			...
			if (res.headersSent) {
				logger.warn(
					{ userId, provider: result.provider },
					"chat: response arrived after HTTP timeout — logging but not sending",
				);
			}
			...
			const tier =
				...
				result.provider === "openrouter-free"
					? "local"
					: "premium";
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 'web', 'ai.chat')`,
			...
			if (result.creditCost > 0) {
				db.prepare(
					"UPDATE users SET credits = MAX(0, credits - ?) WHERE id = ?",
				).run(result.creditCost, userId);
			}
			deductSubscriptionCredits(userId, result.creditCost);

			const updatedCredits =
				(
					db.prepare("SELECT credits FROM users WHERE id = ?").get(userId) as {
						credits: number;
					}
				)?.credits ?? userCredits;
			...
			// Background AI memory extraction (non-blocking)
			extractMemoriesWithAI(userId, message, cleanReply).catch((e: unknown) =>
				logger.debug({ err: e }, "background task failed"),
			);
			// Phase 94: also extract into user_memories (flat key-value store)
			extractMemoriesFromConversation(userId, [
			...
			logActivity(
				String(userId),
				`${chatAgentName} replied`,
				(cleanReply || "").slice(0, 80),
				"bot",
			);
			const chatPersonalityId =
				(effectiveAgentConfig?.personality as string) || "jarvis";
			...
			if (!res.headersSent) {
				res
					.status(500)
					.json({ error: "Failed to process message. Please try again." });
			}
		}
	},
);
...
router.post(
	...
	async (req: AuthRequest, res) => {
		...
		if (cmd === "gs reminders list") {
			...
			if (!reminders.length) {
				res.json({
					output: 'No reminders set. Use: gs reminders add "text"',
					isError: false,
				});
				return;
			}
			const table =
				...
				reminders
					.map(
						(r) =>
							`${(r.id as string).slice(0, 4)} | ${(r.text as string).padEnd(27)} | ${r.datetime || "no date"}${r.completed ? " done" : ""}`,
					)
					.join("\n");
		...
		if (cmd.startsWith("gs reminders add ")) {
			...
			if (!text || text.length > 500) {
				res.json({
					output: "Reminder text required (max 500 chars)",
					isError: true,
				});
				return;
			}
			...
			db.prepare(
				"INSERT INTO reminders (id, user_id, text, channel, category, created_by, scheduled_for) VALUES (?, ?, ?, ?, ?, ?, ?)",
			).run(id, userId, text, "push", "general", "terminal", scheduledFor);
			db.prepare(
				`INSERT INTO activity_log (id, user_id, action, details, icon) VALUES (?, ?, 'Created reminder', ?, 'bell')`,
			).run(uuid(), userId, text);
			res.json({
				output: `Reminder added! ID: ${id.slice(0, 8)}\nText: ${text}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs usage today") {
			const usage = db
				.prepare(
					"SELECT COUNT(*) as calls, SUM(tokens_in) as tin, SUM(tokens_out) as tout, SUM(cost_usd) as cost FROM usage_events WHERE user_id = ? AND date(created_at) = date('now')",
				)
				.get(userId) as Record<string, unknown>;
			res.json({
				output: `Today's Usage:\n  API Calls: ${usage?.calls || 0}\n  Tokens: ${usage?.tin || 0} in / ${usage?.tout || 0} out\n  Cost: $${((usage?.cost as number) || 0).toFixed(4)}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs usage month") {
			const usage = db
				.prepare(
					"SELECT provider, COUNT(*) as calls, SUM(cost_usd) as cost FROM usage_events WHERE user_id = ? AND created_at >= datetime('now', '-30 days') GROUP BY provider",
				)
				.all(userId) as Record<string, unknown>[];
			const total = db
				.prepare(
					"SELECT SUM(cost_usd) as cost FROM usage_events WHERE user_id = ? AND created_at >= datetime('now', '-30 days')",
				)
				.get(userId) as Record<string, unknown>;
			const lines = [
				"This Month:",
				`  Total Cost: $${((total?.cost as number) || 0).toFixed(2)}`,
				"  By Provider:",
			];
			for (const row of usage)
				lines.push(
					`    ${row.provider}: $${((row.cost as number) || 0).toFixed(2)} (${row.calls} calls)`,
				);
		...
		if (cmd === "gs integrations") {
			const integrations = db
				.prepare(
					"SELECT name, status, health, requests_today FROM integrations WHERE user_id = ?",
				)
				.all(userId) as Record<string, unknown>[];
			const lines = integrations.map(
				(i) =>
					`  ${(i.name as string).padEnd(16)} - ${i.status}${i.status === "connected" ? ` (${i.health}% health, ${i.requests_today} req today)` : ""}`,
			);
			res.json({
				output: `Integrations:\n${lines.join("\n")}`,
				isError: false,
		...
		if (cmd === "gs automations") {
			const automations = db
				.prepare(
					"SELECT name, trigger_type, enabled, run_count FROM automations WHERE user_id = ?",
				)
				.all(userId) as Record<string, unknown>[];
			...
			const lines = automations.map(
				(a) =>
					`  ${a.name} [${a.enabled ? "ON" : "OFF"}] trigger: ${a.trigger_type}, runs: ${a.run_count}`,
			);
		...
		if (cmd === "gs status") {
			const agent = db
				...
				.get(userId) as Record<string, unknown>;
			res.json({
				output: `Agent Status: ${agent?.status || "unknown"}\nName: ${agent?.name || "Geek"}\nMode: ${agent?.mode || "builder"}\nVoice: ${agent?.voice || "friendly"}\nModel: ${agent?.primary_model || "default"}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs portfolio") {
			const user = db
				...
				.get(userId) as Record<string, unknown>;
			res.json({
				output: `Portfolio: /portfolio/${user?.username || "you"}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs deploy") {
			db.prepare("UPDATE portfolios SET is_public = 1 WHERE user_id = ?").run(
				userId,
			);
			const user = db
				.prepare("SELECT username FROM users WHERE id = ?")
				.get(userId) as Record<string, unknown>;
			db.prepare(
				`INSERT INTO activity_log (id, user_id, action, details, icon) VALUES (?, ?, 'Deployed portfolio', 'Public', 'globe')`,
			).run(uuid(), userId);
			res.json({
				output: `Portfolio deployed!\nLive at: /portfolio/${user?.username || "you"}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd.startsWith("gs connect ")) {
			...
			const integration = db
				.prepare(
					"SELECT * FROM integrations WHERE user_id = ? AND (LOWER(type) = ? OR LOWER(name) = ?)",
				)
				.get(userId, svc, svc) as Record<string, unknown> | undefined;
			if (integration) {
				db.prepare(
					"UPDATE integrations SET status = 'connected', health = 100, last_sync = ? WHERE id = ?",
				).run(new Date().toISOString(), integration.id);
				db.prepare(
					`INSERT INTO activity_log (id, user_id, action, details, icon) VALUES (?, ?, 'Connected', ?, 'link')`,
				).run(uuid(), userId, integration.name);
				res.json({
					output: `Connected ${integration.name}! Health: 100%`,
					isError: false,
				});
			} else {
				res.json({ output: `Integration "${svc}" not found.`, isError: true });
			}
			return;
		}
		...
		if (cmd.startsWith("gs disconnect ")) {
			...
			if (integration) {
				db.prepare(
					"UPDATE integrations SET status = 'disconnected', health = 0 WHERE id = ?",
				).run(integration.id);
				res.json({
					output: `Disconnected ${integration.name}.`,
					isError: false,
				});
			} else {
				res.json({ output: `Integration "${svc}" not found.`, isError: true });
		...
		if (cmd.startsWith("gs profile set ")) {
			...
			if (column) {
				...
				res.json({
					output: `Unknown field: ${field}. Allowed: ${Object.keys(PROFILE_FIELDS).join(", ")}`,
					isError: true,
				});
			}
			return;
		...
		if (cmd === "gs export") {
			const user = db
				.prepare(
					"SELECT id, email, username, name, bio, location, plan, credits, created_at FROM users WHERE id = ?",
				)
				.get(userId);
			const reminders = db
			...
			res.json({
				output: JSON.stringify(
					{ user, reminders, integrations, portfolio },
					null,
					2,
				),
				isError: false,
		...
		if (cmd === "gs pico list") {
			...
			if (!agents.length) {
				res.json({ output: "No Weebo agents found.", isError: false });
				return;
			}
		...
		if (cmd.startsWith("gs pico create ")) {
			...
			if (!name || name.length > 30) {
				res.json({ output: "Agent name required (1-30 chars)", isError: true });
				return;
			}
			try {
				...
				res.json({
					output: `Created Weebo agent "${agent.name}" at slot ${agent.slot}`,
					isError: false,
				});
			} catch (err) {
				res.json({
					output: err instanceof Error ? err.message : "Failed to create agent",
					isError: true,
				});
			}
			return;
		...
		if (cmd.startsWith("gs pico pause ")) {
			...
			if (isNaN(slot) || slot < 1 || slot > 6) {
				res.json({
					output: "Usage: gs pico pause <slot> (1-6)",
					isError: true,
				});
				return;
			}
			const { getAgentBySlot, updateAgent } = await import(
				"../services/pico-fleet.js"
			);
			const agent = getAgentBySlot(userId, slot);
			if (!agent) {
				res.json({ output: `No agent at slot ${slot}`, isError: true });
				return;
			}
			updateAgent(agent.id, userId, { status: "paused" });
			res.json({
				output: `Paused ${agent.name} (slot ${slot})`,
				isError: false,
			});
			return;
		}
		...
		if (cmd.startsWith("gs pico resume ")) {
			...
			if (isNaN(slot) || slot < 1 || slot > 6) {
				res.json({
					output: "Usage: gs pico resume <slot> (1-6)",
					isError: true,
				});
				return;
			}
			...
			res.json({
				output: `Resumed ${agent.name} (slot ${slot})`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs pico tasks") {
			...
			if (!tasks.length) {
				res.json({
					output: 'No tasks found. Use: gs task "description"',
					isError: false,
				});
				return;
			}
		...
		if (cmd.startsWith("gs task ")) {
			const taskDesc = command
				.slice(8)
				.replace(/^["']|["']$/g, "")
				.trim();
			if (!taskDesc) {
				res.json({ output: 'Usage: gs task "description"', isError: true });
				return;
			}
			...
			try {
				...
				if (sub && sub.credits_remaining < 10) {
					res.json({
						output:
							"Not enough credits for task planning (minimum 10 required)",
						isError: true,
					});
					return;
				}
				...
				const { planTasks, queueTasks } = await import(
					"../services/pico-fleet.js"
				);
				const { tasks, creditCost } = await planTasks(
					userId,
					taskDesc,
					taskUserPlan,
				);
				...
				if (tasks.length === 0) {
					res.json({
						output: `No actionable tasks planned. Credits used: ${creditCost}`,
						isError: false,
					});
					return;
				}
				...
				const summary = tasks
					.map(
						(t) =>
							`  - [slot ${t.agent_slot}] ${t.task_type}: ${t.description}`,
					)
					.join("\n");
				const updatedSub = db
					.prepare(
				...
				res.json({
					output: `Task planning failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					isError: true,
				});
			}
			return;
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 'terminal', 'ai.chat')`,
				...
				res.json({
					output: `[${agentConfig?.name || "Geek"}] Sorry, I couldn't process that request right now. Try again shortly.`,
					isError: true,
				});
			}
			return;
		...
		if (cmd === "gs health") {
			try {
				...
				const lines = Object.entries(health)
					.map(
						([k, v]) =>
							`<span style="color:#7B61FF;font-weight:bold">${k}:</span> ${typeof v === "object" ? JSON.stringify(v) : v}`,
					)
					.join("\n");
				...
				res.json({
					output:
						'<span style="color:#FF6161">Failed to reach health endpoint</span>',
					isError: true,
				});
			}
			return;
		...
		if (cmd === "gs brief") {
			try {
				const { createBriefing: createBriefingFn } = await import(
					"../../../services/daily-briefing.js"
				);
				const content = await createBriefingFn(userId);
				res.json({
					output: `<span style="color:#7B61FF;font-weight:bold">Daily Briefing:</span>\n${content}`,
					isError: false,
				});
			} catch (err) {
				res.json({
					output: `<span style="color:#FF6161">Briefing failed: ${(err as Error).message}</span>`,
					isError: true,
				});
			}
			return;
		...
		if (cmd.startsWith("gs remind ")) {
			...
			const scheduledFor = Date.now() + 3600_000; // Default 1 hour from now
			db.prepare(
				"INSERT INTO reminders (id, user_id, text, channel, category, created_by, scheduled_for) VALUES (?, ?, ?, 'push', 'general', 'terminal', ?)",
			).run(remId, userId, reminderText, scheduledFor);
			res.json({
				output: `<span style="color:#61FF7B">Reminder created:</span> ${reminderText}`,
				isError: false,
			});
			return;
		}
		...
		if (cmd === "gs deploy portfolio") {
			...
			res.json({
				output: '<span style="color:#61FF7B">Portfolio deployed!</span>',
				isError: false,
			});
			return;
		}
		...
		if (cmd === "clear") {
			res.json({ output: "", isError: false, clear: true });
			return;
		}
