...
//
// Defines the agent ecosystem that PicoClaw can dispatch to.
// Each agent has a role, capabilities, preferred model, and
// a system prompt template. Agents can be composed into
// workflows for multi-step task execution.
//
// Agent Types:
//   - analyst:    Deep reasoning, comparisons, trade-offs
//   - coder:      Code generation, debugging, refactoring
//   - planner:    Roadmaps, step-by-step plans, schedules
//   - researcher: Information gathering, summarization
//   - executor:   Automation execution, API calls, triggers
//   - reviewer:   Quality checks, code review, validation
// ============================================================
...
export interface AgentDefinition {
  ...
  maxTokens: number;
  systemPrompt: string;
  canChainTo: AgentRole[];       // Which agents this one can hand off to
  intentKeywords: string[];      // Keywords that suggest this agent
  costMultiplier: number;        // Credit cost multiplier (1.0 = standard)
}
...
/**
 * Score how well an agent matches a given message.
...
export function scoreAgentMatch(role: AgentRole, message: string): number {
  ...
  // Capability matching (secondary signal)
  ...
  // Normalize score (max possible ~ intentKeywords.length * 2 + capabilities.length * 0.5)
...
/**
 * Determine the best agent(s) for a given message.
...
/**
 * Parse DELEGATES section from an agent's response.
 * Returns an array of { role, task } for chaining.
 */
export function parseDelegates(response: string): Array<{ role: AgentRole; task: string }> {
  ...
  for (const line of lines) {
    // Parse "role: task description" or "- role: task description"
...
/**
 * Map a model tier to the actual provider used in the LLM router.
 */
