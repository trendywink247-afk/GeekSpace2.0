/**
 * Database migrations — idempotent ALTER TABLE + CREATE TABLE IF NOT EXISTS.
 *
 * All statements are wrapped in try/catch so they can run on both fresh
 * and existing databases without error.
 *
 * Called from index.ts after core schema is applied.
 */

import type Database from 'better-sqlite3';
import { logger } from '../logger.js';

export async function applyMigrations(db: Database.Database): Promise<void> {
...
// ── GeekOS user agents + Planner blocks ──────────────────────────────────────
