...
export function seedDemoData(db: Database.Database) {
  ...
  if (!hasOriginal) {
    ...
    insertPortfolio.run('demo-1', 'alex', 'Full-stack Developer & AI Enthusiast',
      ...
      JSON.stringify([
        { date: '2026-01-15', title: 'Joined Agentin', description: 'Started the AI OS journey', autoGenerated: true },
  ...
  if (!hasNew) {
    ...
    insertUser.run('demo-9', 'srikar@geekspace.demo', 'srikar', newPass, 'Srikar', 'SK',
      ...
      'Hyderabad, IN', '', 'Developer', 'Agentin',
    ...
    insertAgent.run('agent-11', 'demo-11', 'Scout', 'Guest AI', 'minimal', 'friendly', "You are a helpful AI assistant for an Agentin guest user.");
    ...
    insertPortfolio.run('demo-9', 'srikar', 'AI & Productivity Developer',
      'Exploring the future of AI-powered productivity.',
      'SK', 'Hyderabad, IN', 'Developer', 'Agentin',
    ...
    insertPortfolio.run('demo-10', 'abhi', 'Backend & Cloud Engineer',
      'Building smarter systems one line at a time.',
      'AB', 'Bangalore, IN', 'Engineer', 'Agentin',
      '["Backend","Cloud","Python","Node.js","Docker"]',
      '[]', '[]', '{}', 'classic', 1, defaultVisibility
    );

    insertPortfolio.run('demo-11', 'guest', 'Agentin Explorer',
      'Exploring Agentin as a guest.',
      'GU', '', 'Guest', '',
      '[]', '[]', '[]', '{}', 'classic', 1, defaultVisibility
    );
  ...
  if (memCount === 0) {
    ...
    insertMem.run('demo-1', 'main_project', 'Agentin AI OS', 'manual', 1.0, now, now);
