 ...
 *
 * Streaming route behaviour:
 * 1. Complex/coding/planning intents → ReAct loop with visible thinking steps
 * 2. Simple intents + Ollama up → direct Ollama stream
 * 3. Simple intents + Ollama down → cloud fallback (openrouter-free)
 * 4. Launch mode ("@council" prefix) → multi-agent orchestration
 *
 * Public portfolio chat falls back Groq → openrouter-free and supports escalation
 * to the owner via Telegram when the agent indicates it will check with them.
 */
 ...
 * - `delta`  — incremental token chunk: `{ text: string }`
 * - `done`   — stream complete: `{ requestId, provider, model, creditsUsed }`
 * - `error`  — fatal error: `{ message: string }`
 * - `action` — deferred action result (code gen, image gen): `{ action, result }`
 *
 * **Intent routing:**
 * 1. Complex/coding/planning → ReAct loop with visible thinking steps
 * 2. Simple + Ollama up → direct Ollama stream
 * 3. Simple + Ollama down → cloud fallback (openrouter-free)
 * 4. `@council` prefix → multi-agent orchestration
 *
 * @body message     - User message to send to the agent.
 * @body personality - Optional personality override (e.g. 'weebo', 'edith', 'jarvis').
 *
 * @example
 * ```javascript
 * const es = new EventSource('/api/agent/chat/stream', { method: 'POST', ... });
 * es.addEventListener('delta', e => process.stdout.write(JSON.parse(e.data).text));
 * es.addEventListener('done',  e => console.log('Credits used:', JSON.parse(e.data).creditsUsed));
 * ```
 */
